...
| `SCR-AUTH-RECOVERY` | `ForgotPasswordScreen` | Solicitud, confirmación, errores y regreso al login. | **COMÚN con límites; candidato #215 abierto.** La raíz compartida se mantiene en `ForgotPasswordForm`/`ForgotPasswordScreenHost`; #215 añade anclas comunes `auth.recovery.*`, granularidad común para prefijo/teléfono desde `PhoneInputSection`, tests `commonTest` de `ForgotPasswordViewModel` y `RemotePasswordRecoveryRepository`, contrato Web y fixture UI iOS autenticable. Web suma evidencia focal hermética `WEB-AUTH-RECOVERY-001` mediante bundle Wasm real, `WebAuthRepository`, cuenta ausente, pregunta, reset fixture, retorno a Login, capturas y red externa bloqueada. Android suma evidencia focal hermética `ANDROID-AUTH-RECOVERY-001` sobre `AuthProductHostContent` real: cuenta ausente, pregunta, reset fixture, retorno a Login, capturas y corrección común del wrapper de recovery de `Box` superpuesto a `Column` etiquetada. | No declarar GO hasta recorrer recuperación real Android-Wasm-iOS con cuenta autorizada: consulta de pregunta, cuenta ausente/error, reset reversible, retorno a Login, capturas comparadas y verificación backend/limpieza si se crean o modifican datos. |
