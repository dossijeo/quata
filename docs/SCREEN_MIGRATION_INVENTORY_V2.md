...
| `ProfileScreen.kt` — `ProfileScreen` / Cuenta y SOS | #156 integra `ProfileScreenHost` como raíz Compose común y Android monta esa raíz. | #156 sustituye la composición parcial por el host común; el editor de avatar Web existe con contratos de selección, transformación y subida. | #156 monta `IosProfileHost`/SOS sobre la raíz común con gateway y uploader iOS. | **COMÚN con límites.** Lectura y shell deben completar postflight exacto Android↔Wasm↔iOS. La mutación E2E de avatar Web no está acreditada: no se guardó/subió un avatar desechable, por lo que sigue como capacidad contractual pendiente. El postflight visual/funcional iOS de Cuenta/Perfil/SOS sigue pendiente. |
...
| Cuenta, Perfil y SOS (#156) | `ProfileScreenHost` común ya está integrado en Android, Wasm e iOS; Web incorpora editor de avatar y ambos bordes conservan sus gateways. | Ejecutar el postflight integrado exacto y comparar Android↔Wasm↔iOS. Registrar una subida temporal real y su limpieza, o declarar explícitamente que la mutación no está acreditada. |
