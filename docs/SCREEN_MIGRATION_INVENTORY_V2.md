...
| ID | Estado tras auditoría | Corrección / alcance vigente | Límite preservado | Fuentes |
|---|---|---|---|---|
| `CHAT-MESSAGES` | COMÚN con límites; añadir acuse de entrega y reconexión como criterios separados; lectura requiere trazabilidad antes de exigir cierre. | Añadir acuse de entrega y reconexión; trazar la lectura antes de exigir su cierre. | Paginación profunda, errores de red y permisos de borrado se mantienen. | S11,S17,S01 · [fuente](https://github.com/dossijeo/quata/blob/bd8a73b03b1139024f9e0447b0d452f156267578/app/src/main/java/com/quata/core/navigation/AppNavGraph.kt) |
...
| `CHAT-FAVORITES` | GO en `main` #219; mantener apertura al origen y alta/baja/estado vacío; evidencia no reejecutada. | Mantener apertura al origen y baja/estado vacío como alcance. | No se reejecutó la evidencia de tres plataformas. | S11,S01 · [fuente](https://github.com/dossijeo/quata/blob/bd8a73b03b1139024f9e0447b0d452f156267578/app/src/main/java/com/quata/core/navigation/AppNavGraph.kt) |
...
| `CHAT-FOCUSED-MESSAGE` | GO en `main` #219 para foco; recepción del deep link/cold start/auth se valida en `FLOW-DEEP-LINKS`. | Enlazar a FLOW-DEEP-LINKS propuesto sin reabrir el scroll focal ya validado. | Cold start y sesión expirada no quedan cubiertos por un mensaje enfocado. | S11,S18,S01 · [fuente](https://github.com/dossijeo/quata/blob/bd8a73b03b1139024f9e0447b0d452f156267578/app/src/main/java/com/quata/core/navigation/AppNavGraph.kt) |
| `CHAT-ATTACHMENTS` | COMÚN con límites; mantener picker/upload/playback/documento/download/share separados; evidencia 8af482e8 sigue declarada, no reejecutada. | Mantener separación de picker/upload/playback/documento/download/share. | No se verificaron ejecución nativa ni permisos físicos; 8af482e8 es evidencia declarada. | S01 · [fuente](https://github.com/dossijeo/quata/blob/efd6a805d3b8f744e4d8b2153a0b1a087e88eb53/docs/SCREEN_MIGRATION_INVENTORY_V2.md) |
| `CHAT-AUDIO` | COMÚN con límites; conservar invalidez de estados optimistas; permisos/grabación física/red no recertificados. | No aceptar Play solicitado como Playing ni simular grabación como permiso real. | Estados nativos, grabación física y red no reejecutados. | S01 · [fuente](https://github.com/dossijeo/quata/blob/efd6a805d3b8f744e4d8b2153a0b1a087e88eb53/docs/SCREEN_MIGRATION_INVENTORY_V2.md) |
...
|---|---|---|---|---|---|
| `ACCOUNT-DETAILS` | GO focal · Android/Web/iOS exact-SHA | Cuenta → Mis datos | Nombre, barrio, prefijo/teléfono, carga, validación, guardado, error y recarga desde la superficie común `ProfileScreenHost`. Aceptación ejecutada sobre Product/Evidence SHA `b27f0aa5305eed2f5151806fbf04e491351a6416`: modificar datos de perfiles autorizados desde UI/host real, verificar persistencia remota y lectura tras recarga, restaurar snapshot y cubrir fallo de guardado sin falso éxito mediante contrato común. Manifest: `docs/candidate-attestations/account-details-parity.json`. | No cierra `ACCOUNT-RECOVERY-SECRET`, `ACCOUNT-PASSWORD-LEGACY`, avatar, SOS, logout/borrado de cuenta ni otros subflujos de Cuenta/Perfil. Web/Wasm usa bridge localhost-only con opt-in porque Compose/Wasm no expone nodos DOM estables para el formulario canvas; no se ejecuta sobre una cuenta real sin opt-in. | S08,S13,S14 · [fuente](https://github.com/dossijeo/quata/blob/b27f0aa5305eed2f5151806fbf04e491351a6416/feature/profile/src/commonMain/kotlin/com/quata/feature/profile/presentation/ProfileScreenHost.kt) |
| `ACCOUNT-RECOVERY-SECRET` | VERIFIED_ANDROID · trazabilidad/E2E pendiente | Cuenta → pregunta/respuesta → recuperación | Trazar el productor del secreto y el consumidor de recuperación. Aceptación: Configurar desde producto; leer estado permitido sin exponer el secreto; ejecutar recuperación autorizada y limpiar/restaurar. | No aceptar SQL de preparación como demostración del productor ni asumir el endpoint desplegado actual. | S08,S14,S01 · [fuente](https://github.com/dossijeo/quata/blob/bd8a73b03b1139024f9e0447b0d452f156267578/app/src/main/java/com/quata/feature/profile/presentation/ProfileScreen.kt) |
...
| `FLOW-CONNECTIVITY-PRESENCE` | VERIFIED_ANDROID · aceptación pendiente | Lifecycle y conectividad de shell | Estado de red, refresco, foreground, pausa/reanudación de observadores y acuse de entrega. Aceptación: Interrumpir/reponer red y volver a primer plano; observar estado, reconexión sin duplicados y delivered. | No afirmar cola offline completa ni acuses de lectura sin trazar esos recorridos. | S11,S17,S15 · [fuente](https://github.com/dossijeo/quata/blob/bd8a73b03b1139024f9e0447b0d452f156267578/app/src/main/java/com/quata/core/navigation/AppNavGraph.kt) |
| `PROF-SHARED-CHAT-DOCUMENTS` | VERIFIED_ANDROID · aceptación multiplataforma pendiente | Chat compartido → perfil del interlocutor | Origen real, control por visitante/peer, lector de documento y retorno al perfil. Aceptación: Adjuntar documento mediante Chat, abrirlo desde el perfil entre esos usuarios, verificar restricciones y cleanup. | Puede ser subcaso de FLOW-DOCUMENT-VIEWER; no implica archivos documentales publicados en Feed/Official ni en perfiles públicos. | S07,S06 · [fuente](https://github.com/dossijeo/quata/blob/bd8a73b03b1139024f9e0447b0d452f156267578/app/src/main/java/com/quata/feature/neighborhoods/data/NeighborhoodRepositoryImpl.kt) |
