...
| `SCR-OFFICIAL-EDITOR` | `OfficialPostEditorScreen` | Crear/editar publicacion oficial, adjuntos, traduccion, permisos, publicacion y errores. | **PARCIAL/COMUN con limites.** Candidata `codex/official-editor-rebuild-v2`: el estado quick/advanced, validacion, media, preview, draft, copy comun y plan de traducciones viven en comun; Android, Web e iOS montan `OfficialPostEditorRoot` mediante slots de plataforma. Web usa ruta dedicada `official-editor`; iOS instala `QuataOfficialEditorViewController` desde la factory autenticada. Web/iOS ya delegan `createPosts` a planes PostgREST comunes con publicacion visible (`is_published`, `published_at`) y evidencia REST reversible contra Supabase real con limpieza de residuo cero. | No declarar GO hasta ejecutar Swift host/iOS firmado sobre este SHA, capturar Android-Wasm-iOS en el mismo flujo/sesion y certificar el merge sintetico exacto. |
