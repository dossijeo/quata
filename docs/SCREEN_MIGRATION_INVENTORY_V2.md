...
| `CHAT-FAVORITES` | Ruta/lista de mensajes favoritos, apertura del mensaje/conversación origen y alta/baja de favorito. | Ruta especial común conectada desde Android, Wasm e iOS a `FavoriteMessagesConversationId`; apertura del mensaje origen pasa por `onOpenMessageConversation`. Falta acreditar carga, vacío/error, mutación reversible, retorno y foco visual real en las tres plataformas. |
