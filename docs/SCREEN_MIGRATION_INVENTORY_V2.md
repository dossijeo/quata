...
| `SCR-OFFICIAL-EDITOR` | `OfficialPostEditorScreen` | Crear/editar publicación oficial, adjuntos, traducción, permisos, publicación y errores. | **AUSENTE/PARCIAL.** Se reconstruye desde `main`: el estado canónico de quick/advanced, validación, media, preview, draft y plan de traducciones vive en común y Android lo consume sin perder restauración. Falta conectar una raíz de producto completa en Wasm/iOS y acreditar publicación/errores con backend real o fixture reversible autorizada. | No reutilizar la candidata histórica #161 como estado vigente. Siguiente cierre: host común completo con slots de media/texto por plataforma; no declarar GO hasta publicar/rollback/evidencias Android↔Wasm↔iOS. |
