...
| `SCR-OFFICIAL-EDITOR` | `OfficialPostEditorScreen` | Crear/editar publicacion oficial, adjuntos, traduccion, permisos, publicacion y errores. | **PARCIAL/COMUN con limites.** En preparacion local sobre `codex/official-editor-web-ios-v1`: el estado quick/advanced, validacion, media, preview, draft, copy comun y plan de traducciones viven en comun; Android, Web e iOS montan `OfficialPostEditorRoot` mediante slots de plataforma. Web usa ruta dedicada `official-editor`; iOS instala `QuataOfficialEditorViewController` desde la factory autenticada. La publicacion Web/iOS sigue fail-closed porque `createPosts` aun es unsupported, y faltan fixture reversible, Swift host final y evidencia visual integrada. | No declarar GO hasta conectar/validar publicacion y rollback con backend real autorizado, capturar Android-Wasm-iOS en el mismo flujo/sesion y certificar el merge sintetico exacto. |
