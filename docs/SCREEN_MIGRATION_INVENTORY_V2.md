...
| `ProfileScreen.kt` — `ProfileScreen` | Perfil Android completo; solo partes de layout/formularios/ViewModel están en común. | `WebProfileHost` es una composición propia; las escrituras permanecen en borrador local o no verificadas, aunque haya lectura remota autenticada. | Hay SOS y perfil de miembro aislados; no existe un «Mi perfil» equivalente completo demostrado. | **PARCIAL/AUSENTE P0.** Extraer `ProfileScreenHost`; no presentar borrador local como actualización remota. |
| `CreatePostScreen.kt` — `CreatePostScreen` | Compositor Android completo y pipeline Android. | `WebPostComposerHost` es propio y usa `WebPostComposerPublicationUnavailableRepository`: publicar falla de forma explícita. | `IosPostComposerHost` es propio y usa `IosComposerPublicationUnavailableRepository`; hay picker/previews parciales, publicación no implementada. | **FALLBACK P0.** Extraer el flujo/raíz Android a común e implementar contratos de publicación sin tocar RLS; mantener captura, editor y exportación como servicios de plataforma. |
