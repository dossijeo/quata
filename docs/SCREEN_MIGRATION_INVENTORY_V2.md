...
| `SCR-OFFICIAL-EDITOR` | `OfficialPostEditorScreen` | Crear/editar publicación oficial, adjuntos, traducción, permisos, publicación y errores. | **AUSENTE/PARCIAL.** Se reconstruye desde `main`: el estado canónico de quick/advanced, validación, media, preview, draft y plan de traducciones vive en común; `OfficialPostEditorRoot` concentra el formulario y Android lo consume mediante slots nativos sin perder restauración. Falta conectar la ruta Wasm/iOS y acreditar publicación/errores con backend real o fixture reversible autorizada. | No reutilizar la candidata histórica #161 como estado vigente. Siguiente cierre: host Web/iOS sobre `OfficialPostEditorRoot`; no declarar GO hasta publicar/rollback/evidencias Android↔Wasm↔iOS. |
