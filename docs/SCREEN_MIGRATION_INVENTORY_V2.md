...
Estado base auditado: `main` en `f120cb2d` (29 de julio de 2026).
...
| 4 | `FeedScreen.kt` \| **CutreFeed** | Producto completo: viewport negro, pager vertical, medios, rail, autor, shortcodes, ranking, comentarios, acciones, refresh y paginación. | Existen `FeedViewModel`, `FeedPagerViewportContent`, `FeedReelPagerContent`, `FeedReelPostContent` y casi todas las piezas visuales; no existe aún una raíz completa equivalente a `FeedScreen`. | Monta `FeedBrowserHostContent`: el **CutreFeed**, una lista de tarjetas desechable con renderer multimedia limitado. | Monta el mismo **CutreFeed**, sin renderer multimedia real. No se considera una base que completar. | Mover la orquestación completa de `FeedScreen` a una raíz común. Inyectar media player/image loader, avatar, share, permisos y navegación. Borrar el CutreFeed y hacer que Android, Wasm e iOS consuman la nueva raíz común. | **CUTREFEED — SUSTITUIR P0** |
