...
| 4 | `FeedScreen.kt` — `FeedScreen` | Producto completo: viewport negro, pager vertical, medios, rail, autor, shortcodes, ranking, comentarios, acciones, refresh y paginación. | Existen `FeedViewModel`, `FeedPagerViewportContent`, `FeedReelPagerContent`, `FeedReelPostContent` y casi todas las piezas visuales; no existe aún una raíz completa equivalente a `FeedScreen`. | Monta `FeedBrowserHostContent`: lista/tarjetas, repositorio read-only y renderer multimedia limitado. | Monta el mismo browser fallback sin renderer multimedia real. | Mover la orquestación completa de `FeedScreen` a una raíz común. Inyectar media player/image loader, avatar, share, permisos y navegación. Sustituir y eliminar el browser fallback. Android debe consumir también la nueva raíz. | **FALLBACK P0** |
