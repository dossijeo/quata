...
| `SCR-OFFICIAL-EDITOR` | `OfficialPostEditorScreen` | Crear/editar publicacion oficial, adjuntos, traduccion, permisos, publicacion y errores. | **PARCIAL/COMUN con limites; backend real corregido.** PR #190 integrado en `main` (`99ae1ef4`): Web expone la accion real del editor oficial, conserva la elegibilidad comun (`state.currentUser?.isOfficial == true`) y queda certificado por evidencia hermetica Web con identidad de PR (`--require-pr-identity`). PR #192 integrado en `main` (`ae6af455`): `OfficialPostEditorRoot` incorpora anclas `testTag` comunes, mantiene validacion/fail-closed en `commonMain` y acredita en Web borrador invalido + publicacion denegada sin mutar Supabase real. La evidencia reversible sobre `main` `59fc98f0` detecto el bypass remoto de `official_posts`; PR #194 integro `20260808_0001_official_posts_actor_guard.sql`, luego aplicado remotamente como SQL exacto versionado sin `supabase db push` ni `migration repair`. El postflight `OFFICIAL-EDITOR-REAL-BACKEND-001` paso: cuenta no oficial denegada, cuenta oficial publicada/leida y hard-delete verificado. El estado quick/advanced, validacion, media, preview, draft, copy comun y plan de traducciones viven en comun; Android, Web e iOS montan `OfficialPostEditorRoot` mediante slots de plataforma. Web usa ruta dedicada `official-editor`; iOS instala `QuataOfficialEditorViewController` desde la factory autenticada. El detector de idioma del editor reutiliza FastText compartido en Android/Web/iOS mediante `TextLanguageIdentifier`; Fang/Unknown o errores del modelo publican en el idioma elegido sin generar traducciones automaticas. Candidato local `299973d0`: Web/Wasm acredita publicacion real reversible de texto, imagen y video con lectura backend, Storage/WordPress cleanup y ausencia post-cleanup; iOS restaura el rol oficial desde `community_profiles` autenticado, monta el host real del editor con sesion Keychain sembrada y acredita `OFFICIAL-EDITOR-IOS-REAL-UI-001` con publicacion real reversible por campos avanzados comunes, imagen y video, preview comun, lectura backend, Storage cleanup por API para imagen, WordPress video delete+404, hard-delete exacto y ausencia post-cleanup (`build-reports/ios/official-editor-real-evidence.json`); Android/Web/iOS acreditan permisos UI en el mismo SHA con sesion temporal no oficial, sin publicacion, ausencia de filas temporales y restauracion del rol original (`build-reports/android/official-editor-permissions-evidence.json`, `build-reports/web/official-editor-permissions-evidence.json`, `build-reports/ios/official-editor-permissions-evidence.json`). Android/Web/iOS tambien acreditan el error de validacion comun `official-editor-feedback` ante borrador vacio sin mutacion backend. Android corrige la ruta directa `official/editor` para no montar el editor si la sesion no es oficial. | No declarar GO hasta capturar la comparativa visual Android<->Web<->iOS final del mismo flujo/sesion con limpieza comprobada. Mantener el postflight RLS real en cada cierre y recordar que el historial remoto de migraciones no fue sincronizado. Si se exige un rechazo backend post-apertura, abrirlo como sublane adicional reversible; no sustituye la validacion comun ya acreditada. |
