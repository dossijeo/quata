...
**Corte:** `main` `20d655b61684c426ea60e7def65a2971e43b2f5d`  
**Fecha:** 2026-07-27  
...
| DOC-001 | P0 | PENDIENTE | Reconciliar documentación obsoleta con `main` y el estado remoto real. | Ninguna. | Tablero/evidencia dejan de llamar “candidata” a la ola integrada; RLS-001 consta desplegada mediante `20260726171005`, sin afirmar que SB-07 remoto mutante completo haya pasado. |
| CI-001 | P0 | PENDIENTE | Crear CI obligatoria Web/Wasm y Android para PR, con JDK/Node/Chrome versionados y timeouts explícitos. | Ninguna. | Ejecuta `:web:wasmJsTest`, `:web:wasmJsBrowserDistribution`, `web-browser-smoke.mjs --docmentis`, matriz declarada de unit tests Android/KMP y lint; cualquier exit no cero falla y se publican JUnit, lint, smoke y logs. |
...
| WEB-BUNDLE-001 | P1 | PENDIENTE | Certificar y hacer bloqueante el presupuesto Wasm. | CI-001. | Baseline con hashes de distribución completa del SHA revisado; `wasm-bundle-budget.json` pasa de `proposed` a `approved` en cambio separado; report CI bloquea >1 MiB raw o >256 KiB gzip y archiva JSON. |
...
| JSIR-001 | P1 | PENDIENTE | Retirar por lotes el JS IR que no es producto. | CI verde Web/Android/iOS por lote. | Cero declaraciones propias `js(IR)`, `jsMain/jsTest` y referencias CI/código a `compileKotlinJs`/`jsBrowser*`, comprobado en grafo; o soporte explícito con host, distribución, smoke y CI. |
