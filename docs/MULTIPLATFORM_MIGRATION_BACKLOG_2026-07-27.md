...
| WEB-SHARE-001 | P2 | PARCIAL | El contrato local reproducible usa HTTPS efímero, perfil Chrome persistente, service worker real, reinicio frío, claim único, limpieza IndexedDB y red cerrada; no usa Supabase. | Hosting HTTPS de staging y navegador/dispositivo compatible para instalar realmente la PWA e invocar la hoja Share Target. | Texto/URL/archivos sobreviven cold start, se reclaman una sola vez y se limpia IndexedDB en la PWA instalada real. |
