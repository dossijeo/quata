...
| Chat grupo/SOS (PR #247, Product/Evidence SHA `566a97e9`; admin Web `5a9d42f9`) | COMÚN con límites reducidos | `ChatGroupManagementContent` y `ChatSosLocationContent` exponen anclas comunes `chat.group.*` y `chat.sos.location.*`, `contentDescription` y roles accesibles desde `commonMain`; contratos focales y compilación Chat Android/Wasm/iOS pasan. `CHAT-GROUP-BACKEND-001` ejecuta RPC reales de grupo con Gabrielo/Gabrielu, perfil temporal, toggle invites, alta, rol, expulsión, bloqueo, salida, borrado y cleanup físico cero en `build-reports/chat-group/chat-group-backend-evidence.json`. Evidencias visuales/operativas `--group-sos-only`: Web `build-reports/web/chat-actions-notifications-evidence.json`, Android `build-reports/android/chat-actions-notifications-evidence.json`, iOS `build-reports/ios/chat-group-sos-evidence.json`; las tres pasan sobre el mismo SHA y terminan con residuo físico cero. Web/Wasm `--group-admin-only` pasa en `build-reports/web/chat-group-admin-evidence.json`: alta desde picker común, menú común de miembro, promoción a moderador, verificación DB y cleanup físico cero; manifest `docs/candidate-attestations/chat-group-admin-web.json`. Android abre/verifica el menú por anclas semánticas comunes antes que por texto nativo; el backfill de auto-merge candidate-final queda cubierto por `scripts/backfill-candidate-auto-merge.mjs`. | No declara GO global de `SCR-CHAT`: faltan administración UI profunda equivalente en Android/iOS, retorno/errores/permisos de mapa SOS, bloqueos/expulsión desde superficie visual y estados de error/rollback forzados. Web registra el límite actual de Compose/Wasm: algunos `testTag` no aparecen como DOM/aria, por lo que el replay cae a texto/bounds visibles solo con verificación backend posterior. |
