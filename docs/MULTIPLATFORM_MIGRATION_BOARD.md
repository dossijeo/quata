...
| --- | --- | --- | --- | --- | --- |
| MP-A01 | P0 | Crear `:ios-shared` (o `:shared-app`) como framework umbrella. Mover desde `feature:feed` la exportación iOS de features hermanas y el papel de composition root; Feed vuelve a depender sólo de sus capas inferiores. Superficie: `settings.gradle.kts`, nuevo módulo, `feature/feed/build.gradle.kts`, `iosApp/project.yml`, CI e imports Swift. | MP-A05 debe mantener enlace/XCTest. Migración incremental: no mover pantallas ni reescribir hosts en el mismo lote. | Pendiente | Un solo framework con nombre/exports documentados; `feature:feed` no exporta Auth/Chat/Notifications ni tiene dependencias API de features hermanas; framework, host Swift y XCTest enlazan en CI para el SHA del lote; Android y Wasm siguen compilando. |
| MP-A02 | P0 | Matriz versionada de capacidades por feature y plataforma: `compila`, `exportada`, `compuesta`, `navegable`, `backend_real`, `e2e`; incluir fuente de UI Web para desactivar/etiquetar capacidades parciales, sin fingir éxito local ni ocultar rutas funcionales. Superficie: documento/manifest versionado, `web/**/Main.kt` y hosts con acciones parciales. | Basarse en evidencia, no inferir backend por compilación. Mutaciones Web bloqueadas por contratos/RLS hasta SB-01..SB-07. | En revisión | `core:FeatureCapabilityManifest` v1 aporta owner/evidencia por celda y proyección inyectable. Web consume el registry en Auth, Feed, Chat, Profile, Communities, Official y Composer; muestra origen local/no disponible pero siempre conserva rutas ya cableadas. Tests declaran `e2e=false` hasta evidencia remota. Falta validación cruzada y merge. |
| MP-A03 | P1 | Sustituir `WebProfileRepository` local por el gateway remoto existente (preferencias sólo como caché recuperable), con error y confirmación de servidor antes de comunicar guardado. Superficie: `web/src/wasmJsMain/**/WebProfileRepository.kt`, gateway PostgREST, host/Profile ViewModel y pruebas. | MP-A02. Bloqueado hasta contrato/RLS y datos efímeros Profile/SOS (SB-06); sin pooler, service-role ni cambios de esquema. | Bloqueado externo | Lectura/patch autenticados contra gateway revisado, caché coherente y error visible; E2E SB-06 con snapshot/restauración/limpieza; eliminar falso éxito local o etiquetarlo sólo como caché offline. |
| MP-A04 | P1 | Shell/navegación iOS por verticales: tras umbrella, rutas reales primero Notifications y Official, después Profile/SOS; luego Communities, Composer, Settings, WhatsNew y External Share sin datos Swift de ejemplo. Superficie: `iosApp/iosApp/QuataIosApp.swift`, factories, router/deep links y XCTest/UI. | MP-A01 y MP-A02. Runtime/sesión pública para repositorios reales; push/dispositivo bloqueado por credenciales (SB-08). | Pendiente | Matriz declara nivel real; ruta autenticada, back/deep link y error verificables; XCTest por ruta y E2E donde haya backend. No marcar Profile antes de Notifications/Official salvo dependencia demostrada. |
...
| MP-A07 | P1 | Reducir warnings Wasm/Compose y medir bundle: encapsular `ExperimentalWasmJsInterop`, corregir deprecaciones, presupuesto decreciente y análisis de Skiko/icons/visor/features. Superficie: `wasmJsMain`, Gradle y `scripts/`. | No ocultar warnings ni retirar DocMentis. Gates: `wasmJsBrowserDistribution` y smoke real. | Pendiente | Baseline versionado por SHA; opt-ins localizados; presupuesto falla sólo ante regresión; bundle/smoke verdes, medición Wasm/JS y plan de carga diferida justificado. |
| MP-A08 | P2 | Decidir `js(IR)` frente a Wasm: producto soportado con host/pruebas/publicación propios, o retirada gradual de targets/adaptadores JS sin afectar Wasm. Superficie: Gradle, `jsMain`, CI e inventario. | MP-A07 aporta coste; decisión de producto si JS se conserva. | Pendiente de decisión | ADR; si se conserva, host/smoke/gate JS; si se retira, grafo/CI reducidos con Wasm/Android/iOS verdes. |
