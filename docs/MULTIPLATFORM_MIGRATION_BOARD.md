...
## Foto de control — 2026-07-27

La ola 1 está integrada en `main` mediante PR #46, merge
`587789ff03df0c1b83baa2b6ca74babc4e4d3499`. La ola 2 también está integrada
mediante PR #47, SHA `9cc84dc2a77935ae2b84a7159e435c1ca6f8f220`, ancestro del
corte `main` `ea0322c1`; su rama histórica ya no existe. Sus gates locales
Web/Android están registrados y la CI iOS
[#30210875187](https://github.com/dossijeo/quata/actions/runs/30210875187)
terminó verde. Compilación, smoke y E2E se contabilizan por separado.
...
| Hosts iOS y smoke tests | Parcial | #30210875187 pasó Kotlin/Native, enlace/XCFramework, host Swift + Share Extension, simulador/XCTest y archive sin firma sobre `9cc84dc2`. APNs acredita plumbing, no entrega. External Share no acredita App Group firmado ni dispositivo físico. El Mac virtual con Xcode 16 es incompatible con las bibliotecas Kotlin/Native generadas para Xcode 26. |
...
| MP-A08 | P2 | Decidir `js(IR)` frente a Wasm: producto soportado con host/pruebas/publicación propios, o retirada gradual de targets/adaptadores JS sin afectar Wasm. Superficie: Gradle, `jsMain`, CI e inventario. | MP-A07 aporta coste; decisión de producto si JS se conserva. | Pendiente CI iOS de integración | JSIR-001 retira los 12 módulos restantes (`core`, `designsystem` y diez features): no quedan declaraciones `js(IR)`, source sets/actuals JS ni tareas JS propias; Wasm se conserva. `:web:wasmJsBrowserDistribution`, smoke Web y `:app:compileDebugKotlin` pasan localmente. Falta la CI iOS sobre el SHA integrado antes de cerrar MP-A08. |
...
La rama `codex/next-kmp-convention-plugin-scaffold` introdujo `build-logic` con
`:feature:settings` como piloto; `:feature:auth` es el segundo adoptante. La
convención conserva Wasm y los tres targets iOS; cada módulo conserva su target
Android, source sets y dependencias. JSIR-001 retiró JavaScript IR de la
convención y de los módulos sin host JS.
...
| MP-A14 | P1 | Reconciliar documentación operativa con checks: generar/actualizar evidencia de SHA para iOS, Wasm y hosts, y marcar explícitamente qué es smoke frente a E2E. | MP-A05 y cada CI exacta. | Reconciliado | En `main` `ea0322c1`, `docs/mp-a14-final-evidence.json` separa olas 1 (`587789ff`, PR #46) y 2 (`9cc84dc2`, PR #47) integradas, compilación, smoke y E2E. Web/Android local y CI iOS verde #30210875187 están documentados. No se declara migración completa. |
...
| SB-07 | Dos usuarios aislados, comunidad/post preaprovisionados y purga externa autorizada | Ejecutar `scripts/run-supabase-e2e-sb07.ps1 -AllowExistingTestData -AllowCommunityMutation`: validar post/membresía, crear/listar/borrar comentario y reacción emoji, y comprobar que el segundo JWT no borra el comentario del actor | **Evidencia histórica del hallazgo (2026-07-26):** antes del forward, el outsider borró el comentario del actor y el runner produjo `rls_violation`; el fixture aislado se purgó y verificó sin perfiles/wall/post residuales. **Estado remoto actual:** `20260726171005` está desplegado y el catálogo hardened, pero el SB-07 remoto mutante completo sigue pendiente con fixtures/purga autorizados. Las mutaciones de cliente permanecen `fail-closed` hasta acreditarlo. | Evidencia histórica de fallo seguro y purga; el forward desplegado no equivale al gate remoto completo. No se declara ranking remoto: no existe una ruta persistente consumida por `SupabaseCommunityApi`. |
...
### Reconciliación de evidencia E2E histórica (ejecutada 2026-07-26)
...
de sus propios datos. El fallo SB-07 de esa fecha es evidencia histórica previa
al forward: RLS-001 está desplegada y hardened mediante `20260726171005`, pero
su SB-07 remoto mutante completo permanece pendiente. SB-09/RLS-002 continúa
abierta; RLS-003, RLS-004 y RLS-005 tampoco se declaran cerradas. Las superficies
mutantes afectadas permanecen bloqueadas en cliente. Véase `docs/RLS_FINDINGS.md`.
