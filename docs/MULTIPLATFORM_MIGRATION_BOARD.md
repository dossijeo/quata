...
<!-- Actualizacion operativa 2026-08-12: candidato local a745d83a reduce `FLOW-LEGAL-DOCUMENTS` y `OVR-ABOUT` sin declarar GO: `LegalDocument` concentra catalogo, labels localizados, URLs publicas y assets Android; `QuataLegalDocumentLinksContent` renderiza los enlaces legales de About/UGC en Android/Web/iOS con anclas comunes y contrato `legal-documents-parity-contract.test.mjs` en gates rapidos; Android materializa el DOCX y entra por `DocumentOpenService` en vez de abrir directamente el lector. Falta cerrar paridad funcional: Web/iOS aun abren URL externa y no existe todavia fuente documental local/renderizado/chrome/error/retorno comun para legales. -->
