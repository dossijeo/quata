...
| MP-A02 | P0 | Matriz versionada de capacidades por feature y plataforma: `compila`, `exportada`, `compuesta`, `navegable`, `backend_real`, `e2e`; incluir fuente de UI Web para ocultar/desactivar/etiquetar capacidades parciales, sin fingir éxito local. Superficie: documento/manifest versionado, `web/**/Main.kt` y hosts con acciones parciales. | Basarse en evidencia, no inferir backend por compilación. Mutaciones Web bloqueadas por contratos/RLS hasta SB-01..SB-07. | Pendiente | Matriz revisable por SHA, owner/evidencia por celda; host Web consume una proyección de capability; pruebas cubren Feed, Communities, Profile, Composer, Official y Chat; celdas sin evidencia siguen `false`/pendientes. |
