...
final verde. No hay GO de `SCR-OFFICIAL-EDITOR` hasta completar video iOS, permisos UI/error y comparativa
...
1. Cerrar `SCR-OFFICIAL-EDITOR` desde el candidato local `1a6f7334260447fecaa89fdbab3f3c159defcad4`: Web/Wasm ya acredita publicacion real reversible de texto, imagen y video con limpieza exacta; iOS ya restaura rol oficial, abre el host real del editor y acredita publicacion real reversible por campos avanzados comunes y por imagen con preview comun, lectura backend, Storage cleanup por API, hard-delete exacto y ausencia post-cleanup (`OFFICIAL-EDITOR-IOS-REAL-UI-001`, reporte `build-reports/ios/official-editor-real-evidence.json`). Falta extender iOS a video, permisos y error, cerrar la comparativa visual Android-Wasm-iOS y mantener la deteccion/traduccion del editor en FastText/transporte compartidos.
