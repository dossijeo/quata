...
| Flow emoji en comentarios de perfil (candidato `8536668d`) | COMÚN con límites reducidos | `CommunityProfileCommentInputContent` reutiliza `QuataEmojiCommentTextField` y `CommunityEmojiPanelContent` con anclas comunes `public-profile.comments.*` y `community.emoji.*`. Evidencias locales sobre `8536668d3305e66c1ad829caa03a50b9af6a4f80`: Web/Wasm `build-reports/web/chat-actions-notifications-evidence.json` con captura `web-chat-profile-content-comment-attempt.png`, Android `build-reports/android/chat-actions-notifications-evidence.json`, iOS `build-reports/ios/chat-actions-notifications-evidence.json`; las tres abren perfil desde Chat, recorren galería/visor, abren comentarios, insertan emoji desde el picker, crean comentario real, verifican DB y terminan con cleanup físico cero; manifest `docs/candidate-attestations/flow-emoji-profile-comments.json`. #271 se integró por auto-merge nativo (`e17db978`) y esta rama activa fue rebasada sobre `origin/main` sin conflictos. | No declara GO global de `FLOW-EMOJI` ni `OVR-COMMENTS`: quedan Feed/Official comments, respuesta/paginación/traducción en comentarios, emoji mezclado con enlaces fuera de esta lane, comparativa visual completa del panel y errores/rollback forzados. |
| Feed/Official comentarios con emoji (candidato `f6eb73b8`) | COMÚN con límites reducidos | `QuataFeedActionRail` expone prefijos comunes `feed.action.*` y `official.action.*`, y Feed/Official usan el picker común `CommunityEmojiPanelContent` dentro del panel de comentarios. Evidencias reales sobre `f6eb73b85a68d6ff29fe81b9dbaadaa5f8750283`: Web/Wasm `build-reports/web/feed-official-comments-emoji-evidence.json`, Android `build-reports/android/chat-actions-notifications-evidence.json`, iOS `build-reports/ios/feed-official-comments-emoji-evidence.json`. Las tres crean comentarios Feed y Official desde UI con fixture reversible común, verifican persistencia por DB y terminan con hard cleanup físico cero; manifest `docs/candidate-attestations/feed-official-comments-emoji.json`. El mismo SHA corrige el contrato Wave2 de perfil público para aceptar el helper Web común basado en prefijo sin volver a exigir literales duplicados. | No declara GO global de `FLOW-EMOJI` ni `OVR-COMMENTS`: faltan respuestas, paginación, perfil desde comentario, traductor dentro de comentarios, emoji mezclado con enlaces, estados/errores del selector y rollback forzado. Web/Wasm conserva fallback relativo controlado para `official.action.comments` porque Compose/Wasm no expone ese botón visible como DOM/ARIA estable; queda compensado por anclas comunes posteriores y verificación DB/cleanup. |
