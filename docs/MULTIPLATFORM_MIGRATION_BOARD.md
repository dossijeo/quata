...
| Hosts Web | Parcial, host real | `Main.kt` enruta Auth, Feed, Chat, Official, Notifications, Profile, Settings, Communities, Composer, WhatsNew/About y External Share. El bundle Wasm y el smoke de navegador no autenticado están verificados. `WEB-AUTH-BROWSER-01` acredita una sesión aislada emitida por Supabase, restaurada en Chrome, con `#feed` y una lectura autenticada; siguen pendientes automatizar la entrada en el formulario Compose, E2E de las demás rutas/mutaciones y Push entregado. |
| Límite de runtime Auth Web | Integrado; E2E parcial | Sin `quata-supabase-url`/`quata-supabase-publishable-key`, Auth muestra un diagnóstico accionable que enumera sólo metadatos públicos faltantes y prohíbe claves privilegiadas. El registry declara `backendReal=false`; `wasmJsTest` y el smoke comprueban el límite. `WEB-AUTH-BROWSER-01` también comprobó el caso configurado con clave publicable, bridge de login, persistencia/restauración de sesión y PostgREST desde Chrome; la UI del formulario continúa fuera de ese runner. |
...
| Hosts iOS y smoke tests | Parcial | Profile/SOS (#32), Composer (#36), Official (#35), Notifications (#39), Chat (#40), Communities (#42), WhatsNew/About (#37), Auth (#29), External Share (#41) y Feed (#4) exponen hosts inyectables. Settings queda en candidato `codex/next-ios-settings-shell`: exporta su host Compose, persiste tema/Touch Flow locales con `UserDefaults` y añade smoke de fábrica diferida; no cuenta integrado hasta su CI iOS exacta. XCTest/UI smoke cubre el arranque/rearranque del composition root sin configuración, pero faltan recorridos autenticados y pruebas funcionales de adaptadores reales. |
...
| Profile/SOS | Parcial integrado | UI/estado, acciones inyectables del host SOS iOS, host Web enrutado y adaptador ContactsUI están integrados. MP-A12 conecta el bootstrap autenticado iOS y la ruta SOS interna al shell con el gateway real de lectura; `:app:compileDebugKotlin`, `:feature:profile:compileKotlinWasmJs` y CI iOS exacta #30166431406 pasaron. Persisten mutaciones RLS/E2E y permisos/selección real. |
