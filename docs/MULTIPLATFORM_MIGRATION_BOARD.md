...
| Profile/SOS | En revisión | UI/estado, acciones inyectables del host SOS iOS, host Web enrutado y adaptador ContactsUI están integrados. MP-A12 conecta el bootstrap autenticado iOS y la ruta SOS al shell con el gateway real de lectura; siguen pendientes validación CI, mutaciones RLS/E2E y permisos/selección real. |
...
| MP-A12 | P1 | Cerrar la vertical iOS siguiente a Official/Notifications: Profile/SOS, con ruta autenticada, back/deep link y estados de error desde el shell; sin datos Swift de ejemplo. | MP-A04 Official/Notifications integrado; permisos/contactos reales se prueban aparte. | En revisión | Rama `codex/next-ios-profile-sos-shell`: Profile/SOS se exporta por `QuataShared`, reutiliza la sesión Keychain y el gateway PostgREST de lectura, y la factory queda diferida hasta haber sesión/configuración. XCTest de cola/factory de ruta añadido; no hay deep link público Profile/SOS, por lo que el router lo conserva como ruta autenticada interna. |
