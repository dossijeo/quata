...
1. Cerrar `SCR-OFFICIAL-EDITOR` desde el candidato local `d908839b`: Web/Wasm ya acredita publicacion real reversible de texto, imagen y video con limpieza exacta, e iOS ya restaura rol oficial y abre el host real del editor con preview visible. Falta extender iOS a publicacion real reversible, adjuntos, permisos y error, verificar lectura backend y limpieza, capturar comparativa visual Android-Wasm-iOS y mantener la deteccion/traduccion del editor en FastText/transporte compartidos.
