...
1. Cerrar `SCR-OFFICIAL-EDITOR` desde el candidato local `299973d07ea64a94c870210902316cebddac5c60`: Web/Wasm ya acredita publicacion real reversible de texto, imagen y video con limpieza exacta; iOS ya restaura rol oficial, abre el host real del editor y acredita publicacion real reversible por campos avanzados comunes, imagen y video, con preview comun, lectura backend, Storage cleanup para imagen, WordPress video delete+404, hard-delete exacto y ausencia post-cleanup (`OFFICIAL-EDITOR-IOS-REAL-UI-001`, reporte `build-reports/ios/official-editor-real-evidence.json`). Android/Web/iOS ya acreditan permisos UI con sesion temporal no oficial, rol restaurado y sin mutacion de publicacion. Android/Web/iOS tambien acreditan el error de validacion comun `official-editor-feedback` ante borrador vacio sin mutar backend (`empty_publish_shows_shared_validation_without_backend_mutation`, capturas de validacion en los reportes reales). Falta cerrar la comparativa visual Android-Wasm-iOS final y mantener la deteccion/traduccion del editor en FastText/transporte compartidos; si se requiere un rechazo backend post-apertura, tratarlo como sublane adicional, no como sustituto del error comun ya validado.
