...
<!-- Actualizacion operativa 2026-08-12: candidato local cc3601eb reduce `FLOW-LEGAL-DOCUMENTS` y `OVR-ABOUT` sin declarar GO: `LegalDocument` concentra catalogo, labels localizados, URLs publicas y assets Android/Web/iOS; `QuataLegalDocumentLinksContent` renderiza los enlaces legales de About/UGC en Android/Web/iOS con anclas comunes y contrato `legal-documents-parity-contract.test.mjs` en gates rapidos; Android, Web e iOS resuelven DOCX locales y entran por `DocumentOpenService`. Falta cerrar paridad funcional: evidencia visual/operativa de apertura/cierre/retorno/error Android-Wasm-iOS, entradas Auth/Ajustes y chrome/estado comun del visor documental. -->
