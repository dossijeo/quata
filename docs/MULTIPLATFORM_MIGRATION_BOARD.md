...
| Documentos legales (#241 candidato `e034adbf`) | PARCIAL reducido | `LegalDocument` y `QuataLegalDocumentLinksContent` concentran catalogo, labels, assets locales y enlaces de About/UGC; `SettingsLegalDocumentsSectionContent` reduce Cuenta/Ajustes en Android, Web e iOS. Auth Registro consume el slot común de enlaces legales en Android/iOS y Web usa botones DOM nativos sólo como borde de interacción, manteniendo catálogo, asset y `DocumentOpenService` compartidos. Cuenta/Perfil también monta la sección común en Android, Web e iOS y abre los mismos DOCX locales por `DocumentOpenService`. Evidencia local: Web `build-reports/web/authenticated-browser-e2e-legal-register-2e249536.json` abre `privacy_es.docx` y `child_safety_es.docx`; Android `:app:compileDebugAndroidTestKotlin`; iOS `scripts/build-ios-intel-simulator-signed.sh` pasa en Mac Hyper-V sobre `1f43d441` y de nuevo sobre `e034adbf`; `QuataIosHostUITests.testAuthLaunchFixtureCanColdStartSharedRegisterLegalLinks` verifica `privacy_es.docx`/`child_safety_es.docx`; contratos legales y matriz pasan en `e034adbf`. | No declara GO: falta evidencia visual/operativa Android-Wasm-iOS de Cuenta/Ajustes y chrome/estado/error común del visor documental. |
