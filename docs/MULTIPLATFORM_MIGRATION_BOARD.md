...
| Límite de runtime Auth Web | Integrado arquitectónicamente; E2E pendiente | Sin `quata-supabase-url`/`quata-supabase-publishable-key`, Auth muestra un diagnóstico accionable que enumera sólo metadatos públicos faltantes y prohíbe claves privilegiadas. El registry declara `backendReal=false`; `wasmJsTest` y el smoke comprueban el límite. No acredita login ni Supabase remoto. |
