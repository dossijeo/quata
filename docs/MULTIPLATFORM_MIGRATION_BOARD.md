...
| Flow emoji (candidato `378e4e0c`) | COMÚN con límites reducidos | `CommunityEmojiPanelContent` expone anclas comunes `community.emoji.panel`, `community.emoji.section.*` y `community.emoji.cell.*` con roles accesibles; el render de Chat usa el atlas común para mensajes sin enlaces y conserva el path de enlaces. Evidencia local sobre `378e4e0cdc325321f2e052ea084b59742f12f58e`: Web/Wasm `build-reports/web/chat-composer-emoji-evidence.json` con captura visual `web-chat-composer-sent.png`; Android `build-reports/android/chat-actions-notifications-evidence.json`; iOS `build-reports/ios/chat-actions-notifications-evidence.json`. Las tres pasan con datos reversibles y hard cleanup físico cero; manifest `docs/candidate-attestations/flow-emoji.json`. | No declara GO global de `FLOW-EMOJI`: falta recorrer el picker real en Feed, Official y comentarios, validar emoji mezclado con enlaces, comparar visualmente el panel completo Android-Web-iOS y cubrir estados/errores del selector. La lane Web `--composer-emoji-only` es focal y no sustituye la evidencia de traducción. |
