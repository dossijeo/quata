...
<!-- Actualizacion operativa 2026-08-12: candidato local 26a1e2cf reduce `FLOW-LEGAL-DOCUMENTS` y `OVR-ABOUT` sin declarar GO: `LegalDocument` concentra catalogo, labels, URLs publicas y assets Android; `QuataLegalDocumentLinksContent` renderiza los enlaces legales de About/UGC en Android/Web/iOS con anclas comunes y contrato `legal-documents-parity-contract.test.mjs` en gates rapidos. Falta cerrar paridad funcional: Web/iOS aun abren URL externa, Android abre DOCX embebidos con lector in-app, y no existe todavia fuente documental local/renderizado/chrome/error/retorno comun para legales. -->
