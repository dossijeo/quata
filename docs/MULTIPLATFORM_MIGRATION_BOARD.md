...
| Flow emoji (candidatos `378e4e0c`, Product/Evidence SHA `35e17606`) | COMÚN con límites reducidos | `CommunityEmojiPanelContent` expone anclas comunes `community.emoji.panel`, `community.emoji.section.*` y `community.emoji.cell.*` con roles accesibles; el render de Chat usa el atlas común para mensajes sin enlaces y conserva el path de enlaces. Evidencia local sobre `378e4e0cdc325321f2e052ea084b59742f12f58e`: Web/Wasm `build-reports/web/chat-composer-emoji-evidence.json` con captura visual `web-chat-composer-sent.png`; Android `build-reports/android/chat-actions-notifications-evidence.json`; iOS `build-reports/ios/chat-actions-notifications-evidence.json`. Candidato `35e176067f919f2e4aa2e1fe61d508aac36994c3` añade render común de emoji de catálogo mezclado con enlaces mediante `QuataEmojiLinkifiedText` y lo acredita enviando un marcador mixto real desde Chat en Web/Wasm `build-reports/web/flow-emoji-mixed-links-web-chat-evidence-35e17606.json`, Android `build-reports/android/flow-emoji-mixed-links-android-composer-evidence-35e17606.json` e iOS `build-reports/ios/flow-emoji-mixed-links-ios-chat-evidence-35e17606.json`; las tres evidencias pasan con capturas visuales, poll backend del mensaje exacto y hard cleanup físico cero. Manifests `docs/candidate-attestations/flow-emoji.json` y `docs/candidate-attestations/flow-emoji-mixed-links.json`. | No declara GO global de `FLOW-EMOJI`: falta comparar visualmente el panel completo Android-Web-iOS y cubrir estados/errores del selector y rollback forzado. La lane Web `--composer-emoji-only` es focal y no sustituye la evidencia de traducción. |
| Comentarios perfil/traductor (candidato `44a455b0`) | COMÚN con límites reducidos | `QuataCommentRowContent` expone anclas comunes de autor `*.comments.author.<profileId>` para Feed, Official y perfil público; Feed/Official cierran el sheet antes de abrir el perfil público compartido para evitar superposición Web/Wasm, y comentarios de perfil incorporan trigger de traductor común con FastText en Web/Wasm e iOS. Evidencias finales sobre `44a455b0fbf43748054c3d8f0b772af85fc0fe4b`: Web/Wasm `build-reports/web/feed-official-comments-author-evidence.json` y `build-reports/web/profile-content-comments-translator-evidence.json`; Android `build-reports/android/feed-official-comments-author-evidence.json` y `build-reports/android/profile-content-comments-translator-evidence.json`; iOS `build-reports/ios/feed-official-comments-author-evidence.json` y `build-reports/ios/profile-content-comments-translator-evidence.json`. Las seis pasan con datos reversibles, verificación DB y cleanup físico cero; manifest `docs/candidate-attestations/comments-profile-translator.json`. | No declara GO global de `FLOW-EMOJI`, `FLOW-TRANSLATOR` ni `OVR-COMMENTS`: quedan estados/errores del selector, rollback forzado, emoji mezclado con enlaces y comparativa visual completa. Web/Wasm conserva fallback de bridge documentado sólo tras resolver el anchor semántico de autor, por interceptación de la capa dismiss del sheet. |
| Comentarios respuestas/paginación (candidato `21047158`) | COMÚN con límites reducidos | `QuataCommentRowContent` y `QuataReplyTargetBannerContent` comparten reply target, cancelación y anclas `*.comments.reply.*`/`*.comments.replyTarget.*`; Feed/Official/perfil público envían respuestas reales desde el composer común, preservan comentarios pendientes optimistas hasta que backend devuelve el equivalente remoto y revalidan comentario posterior, poll DB, adjuntos de perfil y cleanup físico cero. Evidencias finales sobre `21047158af6c240ac23e33d4049d196640d37c19`: Web/Wasm `build-reports/web/feed-official-comments-emoji-evidence.json` y `build-reports/web/chat-actions-notifications-evidence.json`; Android `build-reports/android/feed-official-comments-emoji-evidence.json` y `build-reports/android/profile-content-replies-evidence.json`; iOS `build-reports/ios/feed-official-comments-emoji-evidence.json` y `build-reports/ios/chat-actions-notifications-evidence.json`. Manifest `docs/candidate-attestations/comments-replies-pagination.json`. | No declara GO global de `OVR-COMMENTS` ni `FLOW-EMOJI`: quedan estados/errores del selector, rollback forzado, emoji mezclado con enlaces y comparativa visual completa. Android Feed/Official estabiliza el replay con scroll semántico al action tag específico de post; Web/Wasm filtra candidatos ARIA fuera del viewport antes de tap; iOS Profile Content pasó en simulador real Mac Hyper-V sobre el SHA exacto. |
...
| Chat grupo/SOS (PR #247, Product/Evidence SHA `566a97e9`; mapa/retorno `045e8836`; razón no disponible revalidada en `b9283da6`; admin multiplataforma `5c8d5f04`; moderación visual `c65eec1b`) | COMÚN con límites reducidos | `ChatGroupManagementContent` y `ChatSosLocationContent` exponen anclas comunes `chat.group.*` y `chat.sos.location.*`, `contentDescription` y roles accesibles desde `commonMain`; contratos focales y compilación Chat Android/Wasm/iOS pasan. `CHAT-GROUP-BACKEND-001` ejecuta RPC reales de grupo con Gabrielo/Gabrielu, perfil temporal, toggle invites, alta, rol, expulsión, bloqueo, salida, borrado y cleanup físico cero en `build-reports/chat-group/chat-group-backend-evidence.json`. Evidencias visuales/operativas `--group-sos-only`: Web `build-reports/web/chat-actions-notifications-evidence.json`, Android `build-reports/android/chat-actions-notifications-evidence.json`, iOS `build-reports/ios/chat-group-sos-evidence.json`; las tres pasan sobre `b9283da697e7949be4d3144ea9b35fe017bd56f6`, muestran la razón común `permission_denied` como `Ubicación no disponible: permiso denegado` y terminan con residuo físico cero. Web/Wasm, Android e iOS `--group-admin-only` pasan sobre `5c8d5f04117fc62f79938ccb0ab9e645523e5de8`: alta desde picker común, menú común de miembro, promoción a moderador, verificación DB y cleanup físico cero en `build-reports/web/chat-group-admin-evidence.json`, `build-reports/android/chat-group-admin-evidence.json` y `build-reports/ios/chat-group-admin-evidence.json`; manifest `docs/candidate-attestations/chat-group-admin.json`. Web/Wasm, Android e iOS `--group-moderation-only` pasan sobre `c65eec1ba45468193ce833ab812738797108c6ca`: altas desde picker común, expulsión y bloqueo desde menú compartido de miembro, verificación DB (`chat_participants.left_at` y `chat_profile_blocks`) y cleanup físico cero en `build-reports/web/chat-group-moderation-evidence.json`, `build-reports/android/chat-group-moderation-evidence.json` y `build-reports/ios/chat-group-moderation-evidence.json`; manifest `docs/candidate-attestations/chat-group-moderation.json`. Android/iOS resuelven anclas semánticas comunes para picker, checkbox, menú y confirmación; Web registra los fallbacks visuales usados donde Compose/Wasm no expone DOM/ARIA estable y los compensa con verificación backend posterior. El backfill de auto-merge candidate-final queda cubierto por `scripts/backfill-candidate-auto-merge.mjs`; `scripts/validate-candidate-attestation.mjs` falla cerrado si el Product/Evidence SHA deja de ser ancestro tras un rebase apilado; manifest SOS actualizado en `docs/candidate-attestations/chat-group-sos.json`. | No declara GO global de `SCR-CHAT`: faltan rollback forzado, más fallos de proveedor/mapa y cobertura exhaustiva de permisos de sistema fuera de la razón común acreditada. |
...
| iOS simulador | GO funcional suplementario | El postflight de `main` `5d2a52d1` pasó Feed y perfil remoto públicos; auth real ejecutada mediante `.xctestrun` con `QUATA_IOS_AUTH_E2E_FILE`; relanzamiento normal sin reinstalar conserva/restaura sesión; Cuenta/Perfil visual PASS. En #215, `IOS_AUTH_RECOVERY_REAL_UI_GATE_PASSED` ejecutó recovery real con `IosAuthRepository` sobre `9810c142` tras añadir `imePadding()` común a Auth y fijar el idioma de evidencia a español; las capturas muestran copy común y submit visible con teclado. Candidato `301d19fa` ejecuta `testProfileSosFixtureRendersSharedContactsEditorAnchors` en el simulador Mac Hyper-V `48950F56-C309-4AA7-921F-D76C6042AC2C`, resolviendo las anclas nativas `profile.sos.*`, seleccionando contacto fixture y abriendo Mensaje SOS. | CPU-raster no es SLA ni reemplaza CI ARM. SOS ya tiene persistencia remota reducida vía SB-06 en rama apilada; aún faltan 5/5 visual, permisos y errores/rollback forzados en UI. |
...
| Cuenta/Perfil/SOS (#156; candidato SOS `301d19fa`) | COMÚN con límites reducidos | `ProfileScreenHost` común integrado; postflight iOS de Feed/perfil público, auth, relanzamiento y Cuenta/Perfil visual PASS. Candidato legal local pendiente añade en Cuenta la sección común de documentos legales y apertura por `DocumentOpenService`. `SOS-CONTACTS` queda reducido sobre `301d19fab5c1c6f221f1deed14513f126d78e7f2`: anclas comunes `profile.sos.*`, Web/Wasm E2E con bridge localhost-only y capturas landscape, Android instrumented 2/2 e iOS XCTest 1/1 en simulador real. Rama apilada `codex/sos-contacts-remote-limits` Product/Evidence SHA `da5953db75837edb3ea29ec16d96c5ba20982c8d` añade SB-06 real contra Supabase con perfiles aprobados, snapshot/restauración exacta de contactos y reporte redactado, más evidencia visual/operativa del límite compartido 5/5: Web/Wasm E2E selecciona cinco de seis candidatos, Android instrumented 2/2 reusa `toggleEmergencyContactSelection` común, e iOS XCTest real selecciona seis toggles por anclas `profile.sos.contact.toggle.sos-fixture-*` y confirma `5 selected`/`4 selected`; manifest `docs/candidate-attestations/profile-sos-remote-limits.json`. | Avatar Web continúa contractual sin mutación E2E acreditada. SOS no declara GO completo: faltan permisos/contact picker y errores/rollback forzados desde UI. |
