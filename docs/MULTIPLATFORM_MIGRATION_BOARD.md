...
| Documentos legales (candidato local) | PARCIAL reducido | `LegalDocument` y `QuataLegalDocumentLinksContent` concentran catalogo, labels, assets locales y enlaces de About/UGC; candidato legal local pendiente suma `SettingsLegalDocumentsSectionContent` común en Android Cuenta, Web Settings e iOS Settings. Contratos locales y compilación focal Android/Web/iOS pasan antes de congelar. | No declara GO: falta entrada Auth equivalente, evidencia visual/operativa Android-Wasm-iOS de Cuenta/Ajustes y chrome/estado/error común del visor documental. |
