...
2. Completar el candidato #215 para `SCR-AUTH-RECOVERY`: anclas comunes, tests de ViewModel/repositorio, fixture iOS, evidencia Web hermetica `WEB-AUTH-RECOVERY-001` y evidencia Android hermetica `ANDROID-AUTH-RECOVERY-001` ya preparados. La evidencia Android detecto y corrigio en comun el wrapper de recovery de `Box` superpuesto a `Column` etiquetada. Quedan consulta real de pregunta, cuenta ausente/error, reset reversible autorizado, retorno a Login, comparativa visual Android-Wasm-iOS y limpieza/backend sin DDL/RLS/grants.
