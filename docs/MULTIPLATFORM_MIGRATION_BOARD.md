...
2. Completar el candidato #215 para `SCR-AUTH-RECOVERY`: anclas comunes, tests de ViewModel/repositorio, fixture iOS, evidencia Web hermetica `WEB-AUTH-RECOVERY-001` regenerada sobre el candidato sin solape, evidencia Android hermetica `ANDROID-AUTH-RECOVERY-001` y backend real `AUTH-RECOVERY-REAL-BRIDGE-001` ya preparados. La evidencia Android detecto y corrigio en comun el wrapper de recovery de `Box` superpuesto a `Column` etiquetada. El backend real uso Gabrielu autorizado, DB parametrizada reversible para preparar/restaurar secreto y reset temporal con password restaurada; el Edge desplegado aun no acepta `update_recovery_secret` y no se desplego funcion. Quedan recorrido real de UI Android-Wasm-iOS, captura iOS conservada, comparativa visual/accesibilidad y limpieza/backend sin DDL/RLS/grants.
