...
final verde. No hay GO de `SCR-OFFICIAL-EDITOR` hasta completar adjuntos iOS, permisos UI/error y comparativa
...
1. Cerrar `SCR-OFFICIAL-EDITOR` desde el candidato local `44ae3839`: Web/Wasm ya acredita publicacion real reversible de texto, imagen y video con limpieza exacta; iOS ya restaura rol oficial, abre el host real del editor y acredita publicacion real reversible por campos avanzados comunes con lectura backend, hard-delete exacto y ausencia post-cleanup (`OFFICIAL-EDITOR-IOS-REAL-UI-001`, reporte `build-reports/ios/official-editor-real-evidence.json`). Falta extender iOS a adjuntos, permisos y error, cerrar la comparativa visual Android-Wasm-iOS y mantener la deteccion/traduccion del editor en FastText/transporte compartidos.
