...
| Servicios iOS base | Integrado | AVFoundation/deep links (#6), autorización real de cámara con `AVCaptureDevice`, grabación/reproducción/caché de audio AVFoundation, MIME de galería y UTI Office (#14/#16), APNs bridge (#19), Quick Look (#30), Keychain con renovación de sesión (#13), transporte Auth nativo, ContactsUI, thumbnails Quick Look de documentos y thumbnails AVFoundation de vídeo locales. XCTest cubre la admisión determinista de thumbnail de vídeo (URL local, remoto/malformado, ancho, tiempo y fallback de archivo ausente) sin afirmar decodificación de un fixture. Micrófono, Fotos y Archivos siguen explícitamente no disponibles en `PermissionService`. |
...
| MP-A04 | P1 | Shell/navegación iOS por verticales: Notifications/Official, después Profile/SOS; luego Communities, Composer, Settings, WhatsNew y External Share sin datos Swift de ejemplo. | Runtime/sesión pública para repositorios reales; push/dispositivo bloqueado por credenciales (SB-08). | Parcial integrado | `259b5ea` integra Notifications/Official; `4e18882` integra Profile/SOS. `3d496f8` integra Communities como ruta interna autenticada con lectura PostgREST/URLSession y repositorio Chat real; #30170903885 es CI exacta verde (Kotlin, framework, Swift y XCTest) usando watchdog de simulador. No hay deep link público, mutaciones Communities, media, roles ni E2E iOS declarados. Siguiente vertical: Composer. |
...
| --- | --- | --- | --- | --- |
| SB-01 | URL de pooler y exactamente una CA PEM explícita y confiable presentes en el entorno del proceso | Ejecutar `scripts/run-supabase-e2e-sb01.ps1`: catálogo PostgreSQL/Storage y RPC de adaptadores KMP, en transacción `READ ONLY`, sin datos de negocio ni secretos | Bloqueado: falta CA explícita confiable. El runner `de2a8ac` aborta antes de abrir red sin `SUPABASE_DB_TLS_CA_FILE` o `SUPABASE_DB_TLS_CA_PEM`, rechaza modos/parámetros SSL inseguros y fuerza verificación. Sus pruebas locales no son E2E. | Informe local sin secretos, listas `missing` vacías, commit y fecha; guía en `docs/SUPABASE_E2E_SB01.md` |
