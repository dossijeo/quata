...
| Documentos legales (#241 candidato `112fc0e8`) | PARCIAL reducido | `LegalDocument` y `QuataLegalDocumentLinksContent` concentran catalogo, labels, assets locales y enlaces de About/UGC; `SettingsLegalDocumentsSectionContent` reduce Cuenta/Ajustes en Android, Web e iOS. Auth Registro consume el slot común de enlaces legales en Android/iOS y Web usa botones DOM nativos sólo como borde de interacción, manteniendo catálogo, asset y `DocumentOpenService` compartidos. Evidencia local: Web `build-reports/web/authenticated-browser-e2e-legal-register-2e249536.json` abre `privacy_es.docx` y `child_safety_es.docx`; Android `:app:compileDebugAndroidTestKotlin`; iOS `scripts/build-ios-intel-simulator-signed.sh` pasa en Mac Hyper-V sobre `1f43d441` y `QuataIosHostUITests.testAuthLaunchFixtureCanColdStartSharedRegisterLegalLinks` verifica `privacy_es.docx`/`child_safety_es.docx`; `112fc0e8` sólo revisa hashes de matriz tras ese cambio Swift. | No declara GO: falta evidencia visual/operativa Android-Wasm-iOS de Cuenta/Ajustes y chrome/estado/error común del visor documental. |
