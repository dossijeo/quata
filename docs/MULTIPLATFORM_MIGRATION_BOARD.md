...
| P1 | Visor Office integrado Web con DocMentis | Pendiente | `web/`, interop Wasm/JS y host de documentos Web | Integrar `@docmentis/udoc-viewer` solo en Web con carga perezosa, aislamiento de assets/licencia y smoke real de PDF/DOCX/PPTX/XLSX; conservar el fallback seguro de descarga y no filtrar la dependencia a `commonMain`. |
