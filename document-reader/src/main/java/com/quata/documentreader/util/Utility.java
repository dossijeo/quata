...
import com.quata.documentreader.dataType.Model_Main;
    ...
    public static boolean deleteFileOrUri(Context context, Model_Main modelMain) {
        ...
        try {
            if (!modelMain.getPath().isEmpty()) {
                delete = new File(modelMain.getPath()).delete();
                logCatMsg("file Deleted by file method.. " + delete);
            } else if (modelMain.getUriPath().isEmpty()) {
                return false;
            } else {
                delete = context.getContentResolver().delete(Uri.parse(modelMain.getUriPath()), null, null) > 0;
