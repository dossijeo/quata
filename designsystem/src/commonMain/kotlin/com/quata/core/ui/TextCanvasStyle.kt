...
data class TextCanvasPattern(val id: String, val label: String, val start: Color, val end: Color)

data class TextCanvasTypography(val fontSize: TextUnit, val lineHeight: TextUnit, val maxLines: Int)
...
fun textCanvasBrush(seedText: String?, patternId: String? = null): Brush {
    ...
    return Brush.linearGradient(colors = listOf(pattern.start, pattern.end), start = Offset.Zero, end = Offset.Infinite)
}

fun textCanvasPattern(seedText: String?, patternId: String? = null): TextCanvasPattern {
    patternId?.let { selectedId -> textCanvasPatterns.firstOrNull { it.id == selectedId }?.let { return it } }
    var hash = 0
    (seedText?.takeIf { it.isNotEmpty() } ?: "Q").forEach { hash = ((hash shl 5) - hash + it.code) }
...
fun textCanvasTypography(text: String, compact: Boolean = false): TextCanvasTypography {
    val base = when {
        ...
        text.length <= 60 -> if (compact) 28.sp else 34.sp
        text.length <= 140 -> if (compact) 24.sp else 29.sp
        text.length <= 260 -> if (compact) 20.sp else 24.sp
        else -> if (compact) 17.sp else 20.sp
    }
    return TextCanvasTypography(base, (base.value * 1.22f).sp, if (compact) 8 else 10)
}
