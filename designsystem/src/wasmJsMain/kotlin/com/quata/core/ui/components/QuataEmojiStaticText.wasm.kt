...
/** Wasm paints inline atlas cells itself because InlineTextContent children are not rendered. */
@Composable
actual fun QuataEmojiStaticText(value: String, color: Color, fontSize: TextUnit, lineHeight: TextUnit, maxLines: Int, overflow: TextOverflow, fontWeight: FontWeight?, modifier: Modifier) {
    ...
    Box(modifier) {
        Text(inline.text, inlineContent = inline.inlineContent, color = color, fontSize = fontSize, lineHeight = lineHeight, maxLines = maxLines, overflow = overflow, fontWeight = fontWeight, onTextLayout = { layout = it })
