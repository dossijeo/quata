-- Runtime support for the Android Supabase chat migration.
-- Adds direct attachment lookup by peer/community and Storage policies for chat uploads.
...
with check (
    ...
    and (storage.foldername(name))[1] = public.quata_chat_auth_profile_id()::text
);

drop policy if exists chat_attachments_storage_update_own on storage.objects;
create policy chat_attachments_storage_update_own
on storage.objects
for update
to authenticated
using (
