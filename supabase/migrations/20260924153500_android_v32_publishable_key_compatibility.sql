...
-- Supabase publishable keys are authenticated by the API gateway and may reach
-- PostgREST without a JWT role claim. The published Android v32 bundle embeds
-- that public-key form. Keep the existing request signature and accept a
-- missing claim only when the exact publishable-key prefix is present.
