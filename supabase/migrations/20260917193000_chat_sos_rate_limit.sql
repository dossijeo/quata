-- Enforce the product's actor-scoped five-minute SOS cooldown at the only authoritative
-- mutation boundary. The advisory transaction lock prevents concurrent clients from both
-- passing the check before either event is visible.
...
begin
    ...
    perform pg_advisory_xact_lock(hashtextextended('quata-chat-sos:' || v_actor::text, 0));
    v_now := clock_timestamp();

    select max(e.created_at)
    into v_last_sent_at
    from public.chat_sos_events e
    where e.profile_id = v_actor;

    if v_last_sent_at is not null and v_last_sent_at + interval '5 minutes' > v_now then
        v_remaining_millis := greatest(
            1,
            ceil(extract(epoch from (v_last_sent_at + interval '5 minutes' - v_now)) * 1000)::bigint
        );
        return jsonb_build_object(
            'rate_limited', true,
            'remaining_millis', v_remaining_millis,
            'sent', 0,
            'errors', '[]'::jsonb,
            'mode', 'supabase',
