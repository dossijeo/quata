...
test("Turnstile sends trusted IP and rejects failed challenge",async()=>{
 let sent; const ok=await verifyTurnstileChallenge("s","token","1.2.3.4",async(_u,o)=>(sent=o.body,{ok:true,json:async()=>({success:true})}));
 assert.equal(ok,true); assert.equal(sent.get("remoteip"),"1.2.3.4");
 assert.equal(await verifyTurnstileChallenge("s","",null,async()=>{throw Error("called")}),false);
});
