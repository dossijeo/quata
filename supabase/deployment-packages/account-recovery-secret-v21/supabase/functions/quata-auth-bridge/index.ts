...
async function handleRequest(req: Request): Promise<Response> {
  ...
  if (action === "update_recovery_secret") {
    ...
    if (Deno.env.get("QUATA_RECOVERY_SECRET_WRITE_ENABLED") !== "true") {
      return jsonResponse({ error: "recovery_secret_write_disabled" }, 503);
    }
    const question = typeof payload.secret_question === "string" ? payload.secret_question.trim() : "";
    const answer = typeof payload.secret_answer === "string" ? payload.secret_answer.trim() : "";
    if (!question || !answer) return jsonResponse({ error: "recovery_secret_invalid" }, 400);
    const { data: actors, error: actorError } = await admin.from("community_profiles")
      .select("id").eq("auth_user_id", identity.user.id).limit(2);
    if (actorError) throw actorError;
    if (actors?.length !== 1) return jsonResponse({ error: "recovery_profile_unavailable" }, 409);
    // Preserve the format consumed by the published v32 Android client.
    const { data: updated, error: updateError } = await admin.from("community_profiles")
      .update({ secret_question: question, secret_answer: answer })
      .eq("id", actors[0].id).eq("auth_user_id", identity.user.id).select("id");
    if (updateError) throw updateError;
    if (updated?.length !== 1) return jsonResponse({ error: "recovery_profile_unavailable" }, 409);
