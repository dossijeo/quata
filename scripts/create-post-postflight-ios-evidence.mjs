...
const CHECK = "CREATE-POST-POSTFLIGHT-IOS-REAL-001";
...
const report = {
  ...
  attempts: [],
  evidence: {},
  cleanup: { temporaryCredentialsRemoved: false, runtimeConfigRestored: false, publishCallbacksInvoked: false },
  steps: [],
};
...
try {
  ...
  localCredentials = join(await mkdirTemp("quata-ios-create-post-postflight-credentials-"), "credentials.json");
  await writeFile(
    localCredentials,
    `${JSON.stringify({
  ...
  remoteCredentials = (await runCapture("ssh", [options.host, "mktemp /tmp/quata-ios-create-post-postflight-credentials.XXXXXX"])).trim();
...
`);
  ...
  console.log(`Create Post postflight iOS evidence written: ${options.output}`);
}

if (report.status !== "passed") {
  console.error(`Create Post postflight iOS evidence failed: ${report.error ?? "unknown"}.`);
  process.exitCode = 1;
} else {
  console.log("Create Post postflight iOS evidence passed.");
}
...
export QUATA_IOS_CREATE_POST_POSTFLIGHT_UI_LOG_DIR=${shellQuote(options.remoteLogDir)}
export QUATA_IOS_CREATE_POST_POSTFLIGHT_UI_RESULT_BUNDLE_DIR=${shellQuote(`${options.remoteLogDir}/xcresults`)}
bash scripts/run-ios-create-post-postflight-ui-test.sh
`);
    return { source: "create-post-postflight", outcome: "success", status: "passed", remoteLogDir: options.remoteLogDir };
  } catch (error) {
    return { source: "create-post-postflight", outcome: "success", status: "failed", remoteLogDir: options.remoteLogDir, error: safeFailure(error) };
  }
}
...
function parseArgs(args) {
  const parsed = {
    ...
    remoteLogDir: process.env.QUATA_IOS_CREATE_POST_POSTFLIGHT_UI_LOG_DIR?.trim() || "build/reports/ios/CREATE-POST-POSTFLIGHT-ui",
    output: join("build-reports", "ios", "create-post-postflight-evidence.json"),
    evidenceDir: join("build-reports", "ios", "create-post-postflight-evidence"),
...
async function prepareRemotePublicRuntimeConfig({ host, project }) {
  const backupPath = (await runCapture("ssh", [host, "mktemp /tmp/quata-ios-create-post-postflight-runtime.XXXXXX"])).trim();
