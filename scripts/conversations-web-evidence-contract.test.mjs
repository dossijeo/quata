...
test("Android and iOS conversation creation prove private reuse and exact group creation", async () => {
  ...
  assert.match(iosUi, /private func typePickerText[\s\S]*?for character in value[\s\S]*?fieldValue\(field\) != expectedPrefix[\s\S]*?must retain the complete typed value across Compose recompositions/);
