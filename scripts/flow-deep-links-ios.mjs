...
export async function executeDeepLinkIosTrial({client,serviceKey,root,macRoot,products,privateDirectory,supabaseCli,expected}) {
  ...
  const preflight=async()=>{
    ...
    if(Object.keys(native).sort().join(",")!==Object.keys(expected.nativeBuild).sort().join(",")||
       ...
       native.builtBackendUrlSha256!==hash(backendUrl)||native.builtPublicKeySha256!==hash(publicKey))return false;
  ...
  try {
    ...
    return {...report,productSha:expected.productSha,nativeBuild:expected.nativeBuild};
