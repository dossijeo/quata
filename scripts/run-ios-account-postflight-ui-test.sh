#!/usr/bin/env bash
# Opt-in local gate: seed a real Keychain session, then exercise Account without destructive confirmation.
set -euo pipefail
...
mkdir -p "$QUATA_IOS_ACCOUNT_POSTFLIGHT_UI_LOG_DIR"
...
run_and_require() {
  ...
  local result_args=()
  if [[ -n "$QUATA_IOS_ACCOUNT_POSTFLIGHT_UI_RESULT_BUNDLE_DIR" ]]; then
    mkdir -p "$QUATA_IOS_ACCOUNT_POSTFLIGHT_UI_RESULT_BUNDLE_DIR"
    local bundle="$QUATA_IOS_ACCOUNT_POSTFLIGHT_UI_RESULT_BUNDLE_DIR/${method}.xcresult"
    rm -rf "$bundle"
    result_args=(-resultBundlePath "$bundle")
  fi
  run_bounded "$method" "$QUATA_IOS_ACCOUNT_POSTFLIGHT_UI_TIMEOUT_SECONDS" "$log" \
