  ...
  for (const [sourceText, anchors] of [
    ...
    for (const [constant, tag] of anchors) {
      ...
      if (constant === "ChatMediaAttachmentTestTag") {
        ...
        assert.match(sourceText, /Surface\([\s\S]*?modifier = Modifier[\s\S]*?\.size\(62\.dp\)[\s\S]*?\.clearAndSetSemantics \{\}/);
        assert.doesNotMatch(sourceText, /Surface\([\s\S]*?onClick = onOpen/);
...
test("focused chat deep links keep attachments away from the viewport edge", () => {
  ...
  assert.match(commonAttachmentPresentation, /\.semantics\(mergeDescendants = false\) \{\s*testTag = primaryTestTag\s*contentDescription = semanticAnchor/s);
  assert.match(commonAttachmentPresentation, /Surface\([\s\S]*?modifier = Modifier[\s\S]*?\.size\(62\.dp\)[\s\S]*?\.clearAndSetSemantics \{\}/);
...
test("iOS media overlay close is exposed through a native accessibility anchor", () => {
  ...
  assert.match(commonAttachmentPresentation, /val primaryTestTag = if \(mediaOwnsOpen\) openButtonTestTag else semanticTestTag/);
  assert.match(commonAttachmentPresentation, /\.clearAndSetSemantics \{\}/);
  assert.doesNotMatch(fullscreenMediaOverlay, /\.clickable\(onClick = onBack\)/);
...
test("audio attachment player exposes stable common playback anchors", () => {
  ...
  assert.match(iosUiTest, /tapSemanticAudioControl\(audioToggle, audioName: audioName, context: "start native playback", in: app\)/);
  ...
  assert.match(iosUiTest, /tapSemanticAudioControl\(pauseToggle, audioName: audioName, context: "pause native playback", in: app\)/);
...
test("common chat product routes attachments and audio without platform-specific product forks", () => {
  ...
  assert.match(androidChatAttachmentAudioPlayerService, /class AndroidChatAttachmentDocumentOpenService/);
  assert.match(androidChatAttachmentAudioPlayerService, /fun interface AndroidChatAttachmentFileResolver/);
  ...
  assert.match(androidChatAttachmentAudioPlayerService, /AndroidChatAttachmentFileCacheResolver/);
  assert.match(appContainer, /AndroidChatAttachmentAudioPlayerService/);
  assert.match(appContainer, /AndroidChatAttachmentDocumentOpenService/);
  assert.match(appContainer, /AndroidChatAttachmentFileCacheResolver/);
  assert.match(iosAvPlayerAudioEngine, /activePlayer\.currentTime = Double\(boundedMillis\) \/ 1_000\.0/);
