...
test("audio attachment player exposes stable common playback anchors", () => {
  ...
  assert.match(iosAvPlayerAudioEngine, /private var item: AVPlayerItem\?/);
  assert.match(iosAvPlayerAudioEngine, /private var playbackStartWatchdog: DispatchWorkItem\?/);
  assert.match(iosAvPlayerAudioEngine, /private var periodicTimeObserver: Any\?/);
  assert.match(iosAvPlayerAudioEngine, /private var statusObservation: NSKeyValueObservation\?/);
  assert.match(iosAvPlayerAudioEngine, /private var timeControlObservation: NSKeyValueObservation\?/);
  ...
  assert.match(iosAvPlayerAudioEngine, /let nextItem = AVPlayerItem\(url: url\)/);
  assert.match(iosAvPlayerAudioEngine, /let nextPlayer = AVPlayer\(playerItem: nextItem\)/);
  assert.match(iosAvPlayerAudioEngine, /installObservers\(for: nextPlayer, item: nextItem, generation: requestGeneration\)/);
...
test("common chat product routes attachments and audio without platform-specific product forks", () => {
  ...
  assert.match(iosAvPlayerAudioEngine, /activePlayer\.seek\(\s*to: CMTime\(seconds: Double\(boundedMillis\) \/ 1_000\.0, preferredTimescale: 600\)/);
