...
test("Android and iOS runners expose an opt-in attachments/audio evidence stage", () => {
  ...
  assert.match(iosUiTest, /audioToggle\.coordinate\(withNormalizedOffset: CGVector\(dx: 0\.5, dy: 0\.5\)\)\.tap\(\)/);
  assert.match(iosUiTest, /clearIosAudioEvidenceEvents\(\)/);
  assert.match(iosUiTest, /waitForIosAudioEvidenceEvent\(audioName: audioName, event: "playing", timeout: 15\)/);
  assert.match(iosUiTest, /waitForIosAudioEvidenceProgress\(audioName: audioName, timeout: 20\)/);
  assert.match(iosUiTest, /waitForIosAudioEvidenceEvent\(audioName: nextAudioName, event: "playing", timeout: 12\)/);
  assert.match(iosUiTest, /UIPasteboard\.general\.string/);
