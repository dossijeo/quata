...
: "${QUATA_IOS_POST_COMPOSER_IMAGE_EDITOR_FIXTURE_OPT_IN:?Set QUATA_IOS_POST_COMPOSER_IMAGE_EDITOR_FIXTURE_OPT_IN.}"
: "${QUATA_IOS_POST_IMAGE_EDITOR_UI_LOG_DIR:=build/reports/ios/POST-IMAGE-EDITOR-ui}"
: "${QUATA_IOS_POST_IMAGE_EDITOR_UI_TIMEOUT_SECONDS:=240}"
: "${QUATA_IOS_POST_IMAGE_EDITOR_UI_RESULT_BUNDLE_DIR:=}"
...
mkdir -p "$QUATA_IOS_POST_IMAGE_EDITOR_UI_LOG_DIR"
...
run_bounded() {
  ...
  if [[ "$status" -eq 124 ]]; then
    ...
    } > "$QUATA_IOS_POST_IMAGE_EDITOR_UI_LOG_DIR/${label}-timeout-diagnostics.log"
  fi
  return "$status"
}

run_bounded bootstatus 120 "$QUATA_IOS_POST_IMAGE_EDITOR_UI_LOG_DIR/bootstatus.log" \
...
def patch_target(target, hint=''):
    ...
    if 'QuataIosUITests' in name:
        env['QUATA_IOS_POST_IMAGE_EDITOR_UI_E2E'] = '1'
        for key in [
...
run_and_require() {
  ...
  local result_args=()
  if [[ -n "$QUATA_IOS_POST_IMAGE_EDITOR_UI_RESULT_BUNDLE_DIR" ]]; then
    mkdir -p "$QUATA_IOS_POST_IMAGE_EDITOR_UI_RESULT_BUNDLE_DIR"
    local result_bundle="$QUATA_IOS_POST_IMAGE_EDITOR_UI_RESULT_BUNDLE_DIR/${method}.xcresult"
    rm -rf "$result_bundle"
    result_args=(-resultBundlePath "$result_bundle")
  fi
  run_bounded "$method" "$QUATA_IOS_POST_IMAGE_EDITOR_UI_TIMEOUT_SECONDS" "$log" \
