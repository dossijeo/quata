...
: "${QUATA_IOS_POST_PICKER_CAMERA_UI_LOG_DIR:=build/reports/ios/post-picker-camera-ui}"
: "${QUATA_IOS_POST_PICKER_CAMERA_UI_TIMEOUT_SECONDS:=240}"
: "${QUATA_IOS_POST_PICKER_CAMERA_UI_RESULT_BUNDLE_DIR:=}"
...
mkdir -p "$QUATA_IOS_POST_PICKER_CAMERA_UI_LOG_DIR"
...
run_bounded() {
  ...
  if [[ "$status" -eq 124 ]]; then
    ...
    } > "$QUATA_IOS_POST_PICKER_CAMERA_UI_LOG_DIR/${label}-timeout-diagnostics.log"
  fi
  return "$status"
}

run_bounded bootstatus 120 "$QUATA_IOS_POST_PICKER_CAMERA_UI_LOG_DIR/bootstatus.log" \
...
def patch_target(target, hint=''):
    ...
    if 'QuataIosUITests' in name:
        env['QUATA_IOS_POST_PICKER_CAMERA_UI_E2E'] = '1'
        for key in [
...
run_and_require() {
  ...
  local result_args=()
  if [[ -n "$QUATA_IOS_POST_PICKER_CAMERA_UI_RESULT_BUNDLE_DIR" ]]; then
    mkdir -p "$QUATA_IOS_POST_PICKER_CAMERA_UI_RESULT_BUNDLE_DIR"
    local result_bundle="$QUATA_IOS_POST_PICKER_CAMERA_UI_RESULT_BUNDLE_DIR/${method}.xcresult"
    rm -rf "$result_bundle"
    result_args=(-resultBundlePath "$result_bundle")
  fi
  run_bounded "$method" "$QUATA_IOS_POST_PICKER_CAMERA_UI_TIMEOUT_SECONDS" "$log" \
...
ui='QuataIosUITests/QuataIosAuthenticatedPostPublishUITests/testAuthenticatedSessionExercisesMediaSourceActionsFromCommonComposer'
run_and_require "$seed" testSeedAuthenticatedSessionForVisualGates "$QUATA_IOS_POST_PICKER_CAMERA_UI_LOG_DIR/seed.log"
run_and_require "$ui" testAuthenticatedSessionExercisesMediaSourceActionsFromCommonComposer "$QUATA_IOS_POST_PICKER_CAMERA_UI_LOG_DIR/ui.log"
echo "IOS_POST_PICKER_CAMERA_UI_GATE_PASSED" >&2
