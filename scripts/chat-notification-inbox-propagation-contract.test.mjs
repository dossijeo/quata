...
test("Android drives the real mute action and shared Notifications repository", () => {
  ...
  assert.match(androidUi, /ChatGroupMenuMuteTestTag else ChatGroupMenuUnmuteTestTag/);
  assert.match(androidUi, /listOf\("Avisos", "Alerts", "Notifications"\)/);
  assert.match(androidUi, /By\.descContains\(label\)/);
