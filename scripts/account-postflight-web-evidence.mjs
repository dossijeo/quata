...
try {
  ...
  console.log(`Account postflight Web evidence written: ${options.output}`);
}

if (report.status !== "passed") {
  console.error(`Account postflight Web evidence failed: ${report.error ?? "unknown"}.`);
  process.exitCode = 1;
} else {
  console.log("Account postflight Web evidence passed.");
}

async function runAttempt(context, session, backend) {
  ...
  page.on("request", (request) => {
    if (observeMutations && request.url().startsWith(backend.url) && !["GET", "HEAD", "OPTIONS"].includes(request.method())) {
      mutationRequests.push({ method: request.method(), path: new URL(request.url()).pathname });
    }
  });
  try {
    await page.goto(`${server.origin}/?quata-account-postflight-e2e=1&quata-auth-e2e=1#profile`, { waitUntil: "domcontentloaded", timeout: 60_000 });
    anchors.ugcTerms = await passUgcTermsGate(page);
    await waitForProfile(page);
    observeMutations = true;
    await waitForPostflightCanvas(page, "Overview");
    evidence.overview = await screenshot(page, "web-account-postflight-overview");
    ...
    evidence.management = await screenshot(page, "web-account-postflight-management");
    anchors.openDeactivate = await invokeAccountPostflightBridge(page, "openDeactivateConfirmation");
...
async function waitForProfile(page) {
  ...
  await page.locator("html[data-quata-account-postflight-bridge='ready']").waitFor({ state: "attached", timeout: 45_000 });
}
...
async function invokeAccountPostflightBridge(page, method) {
  await page.locator("html[data-quata-account-postflight-bridge='ready']").waitFor({ state: "attached", timeout: 45_000 });
  await page.evaluate((method) => {
    const bridge = globalThis.__quataAccountPostflightE2EProduct;
    if (!bridge || typeof bridge[method] !== "function") throw new Error(`account_postflight_bridge_missing:${method}`);
    bridge[method]();
  }, method);
  await delay(350);
  return { kind: "webE2eBridge", value: `__quataAccountPostflightE2EProduct.${method}` };
}

async function waitForPostflightState(page, expectedPage, expectedConfirmation) {
  await page.waitForFunction(({ expectedPage, expectedConfirmation }) => {
    globalThis.__quataAccountPostflightE2EProduct?.snapshot?.();
    const element = document.documentElement;
    return element.getAttribute("data-quata-account-postflight-page") === expectedPage &&
...
async function waitForPostflightCanvas(page, expectedPage) {
  ...
  while (Date.now() < deadline) {
    if (await page.evaluate((pageName) => {
      globalThis.__quataAccountPostflightE2EProduct?.snapshot?.();
      return document.documentElement.getAttribute("data-quata-account-postflight-page") === pageName;
    }, expectedPage)) {
  ...
  throw new Error("account_postflight_visual_not_ready");
}
...
async function login(backend, credentials, clientInstanceId) {
  const response = await fetch(`${backend.url}/functions/v1/quata-auth-bridge`, {
    method: "POST",
    headers: { apikey: backend.key, "content-type": "application/json", "x-client-info": "quata-account-postflight-web-evidence" },
    body: JSON.stringify({
      action: "web_login",
