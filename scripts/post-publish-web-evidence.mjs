...
try {
  ...
  report.diagnostics = {
    ...(report.diagnostics ?? {}),
    bridgeBeforeDestination: await postComposerBridgeState(page),
    composerStateBeforeDestination: await postComposerProductState(page),
    destinationCandidates: await semanticCandidates(page, `composer-destination-option.${destination.wallId}`),
  };
...
async function fillSemanticInput(page, id, value) {
  ...
  }, { selector: `[id=${cssString(id)}]`, expected: value.slice(0, 24) }, { timeout: 15_000 }).catch(() => {});
}
