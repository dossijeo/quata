#!/usr/bin/env bash
# Opt-in local gate: seed a real Keychain session, then execute the normal-launch Chat favorites/focus UI test.
set -euo pipefail
...
timeout_diagnostics() {
  local label="$1" diagnostics="$QUATA_IOS_CHAT_FAVORITES_FOCUSED_LOG_DIR/${label}-timeout-diagnostics.log"
  {
...
run_bounded bootstatus 120 "$QUATA_IOS_CHAT_FAVORITES_FOCUSED_LOG_DIR/bootstatus.log" \
...
def patch_target(target, hint=''):
    ...
    if 'QuataIosUITests' in name:
        env['QUATA_IOS_CHAT_FAVORITES_FOCUSED_UI_E2E'] = '1'
