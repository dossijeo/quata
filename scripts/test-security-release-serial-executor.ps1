...
try {
    ...
    Fixture
    $dry = Exec @("--action", "dry-run"); Assert-True ($dry.code -eq 0) "dry-run failed"; $fp1 = (($dry.output[-1] | ConvertFrom-Json).migrations | Where-Object version -eq "20260726171001").preconditionSha256
    ...
    $dry2=Exec @("--action", "dry-run"); $fp2=((($dry2.output[-1]|ConvertFrom-Json).migrations|Where-Object version -eq "20260726171002").preconditionSha256); $gate=Join-Path $temp "gates.json"; @{migration="20260726171001";status="passed";preconditionSha256=$fp1}|ConvertTo-Json|Set-Content $gate
    $two=Exec @("--action","apply-002","--expected-precondition-sha256",$fp2,"--gate-evidence",$gate); Assert-True ($two.code -eq 0) "002 failed: $($two.output -join "`n")"
...
} finally { $env:NODE_PATH=$oldNodePath; docker rm -f $container *> $null; if(Test-Path $temp){Remove-Item $temp -Recurse -Force} }
