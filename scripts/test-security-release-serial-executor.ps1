...
try {
    ...
    $dry2=Exec @("--action", "dry-run"); $fp2=((($dry2.output[-1]|ConvertFrom-Json).migrations|Where-Object version -eq "20260726171002").preconditionSha256); $gate=Join-Path $temp "gates.json"; $shaA=('a'*64); @{schemaVersion=1;releaseCommit=('b'*40);snapshotFingerprint=('c'*64);migration="20260726171001";status="passed";preconditionSha256=$fp1;reports=@{dbReleaseSafety=@{status="passed";sha256=$shaA};backendCompatibility=@{status="passed";sha256=$shaA};sb07=@{status="passed";sha256=$shaA}}}|ConvertTo-Json -Depth 4|Set-Content $gate
