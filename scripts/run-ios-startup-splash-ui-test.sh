...
mkdir -p "$QUATA_IOS_STARTUP_SPLASH_UI_LOG_DIR"
...
run_bounded() {
  ...
  if [[ "$command_status" -eq 124 ]]; then
    ...
    } > "$QUATA_IOS_STARTUP_SPLASH_UI_LOG_DIR/${label}-timeout-diagnostics.log"
  fi
  return "$command_status"
}

set +e
run_bounded bootstatus 120 "$QUATA_IOS_STARTUP_SPLASH_UI_LOG_DIR/bootstatus.log" \
...
if [[ "$xcode_status" -ne 0 ]]; then
  if [[ "$xcode_status" -eq 124 ]] && grep -q 'simctl diagnose' "$QUATA_IOS_STARTUP_SPLASH_UI_LOG_DIR/ui.log"; then
    ...
    grep -q '\*\* TEST EXECUTE SUCCEEDED \*\*' "$QUATA_IOS_STARTUP_SPLASH_UI_LOG_DIR/ui.log" || exit "$xcode_status"
  fi
fi
