...
const deviceCredentialsFileName = "account-details-credentials.json";
const deviceCredentialsPath = `app-internal:${deviceCredentialsFileName}`;
const appFilesDir = "files";
const deviceEvidencePath = `${appFilesDir}/account-details-evidence`;
...
try {
  ...
  localCredentials = join("build-reports", "android", `account-details-credentials-${randomUUID()}.json`);
  ...
  const instrumentationOutput = await runCapture(adb, [
    ...
    "-e", "class", "com.quata.feature.profile.presentation.ProfileDetailsRealInstrumentedTest#authenticatedUserUpdatesAccountDetailsFromCommonProfile",
    "-e", "quataAccountDetailsCredentialsFile", deviceCredentialsPath,
    "-e", "quataAccountDetailsEvidence", "1",
    "com.quata.test/androidx.test.runner.AndroidJUnitRunner",
  ]);
  const attempt = { source: "profile-account-details", outcome: "success", instrumentationTail: redactedTail(instrumentationOutput) };
  ...
  await verifyAndroidProfileRestored(evidenceDir);
  ...
  console.log(`Account details Android evidence written: ${options.output}`);
}

if (report.status !== "passed") {
  console.error(`Account details Android evidence failed: ${report.error?.message ?? report.error ?? "unknown"}.`);
  process.exitCode = 1;
} else {
  console.log("Account details Android evidence passed.");
}

function parseArgs(args) {
  const parsed = {
    output: resolve(join("build-reports", "android", "account-details-evidence.json")),
    evidenceDir: resolve(join("build-reports", "android", "account-details-evidence")),
    credentialsFile: process.env.QUATA_ACCOUNT_DETAILS_CREDENTIALS_FILE?.trim() || DEFAULT_CREDENTIALS_FILE,
  };
...
async function verifyAndroidProfileRestored(evidenceDir) {
  const platformReportPath = join(evidenceDir, "android-account-details-evidence.json");
  const platformReport = JSON.parse(await readFile(platformReportPath, "utf8"));
  report.accountDetailsSteps = [
    "details_opened",
    "fields_edited",
    "profile_saved",
    "remote_persisted",
  ...
  if (platformReport?.remotePersisted !== true) throw new Error("android_account_details_remote_not_persisted");
  if (platformReport?.reloadVerified !== true) throw new Error("android_account_details_reload_not_verified");
  if (platformReport?.cleanup?.profileRestored !== true) throw new Error("android_account_details_cleanup_not_verified");
}
