-- Read-only semantic audit for the versioned shared-attachment listing function.
...
with target as (
  ...
  where n.nspname = 'public'
    and p.proname = 'quata_chat_list_shared_attachments'
    and pg_get_function_identity_arguments(p.oid) =
      'p_actor_profile_id uuid, p_peer_profile_id uuid, p_thread_id bigint, p_community_id uuid, p_limit integer, p_offset integer, p_kind text'
), mismatches as (
  select count(*) as mismatch_count
  from target
  where function_name is distinct from 'quata_chat_list_shared_attachments'
     ...
     or definition_md5 is distinct from 'eb2785d0843a52431a7b3e2e58a3bddc'
