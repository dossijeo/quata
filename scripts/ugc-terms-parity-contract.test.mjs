...
test('UGC terms use one common local-first contract across platforms', () => {
  ...
  assert.match(gateContent, /gateway\.flushPendingAcceptanceIfSupported\(\)/);
...
test('Web and iOS call the same Supabase RPCs with canonical parameter names', () => {
  ...
  assert.match(androidAuthRepository, /clearLocalAccountData\(profileId\)[\s\S]*sessionManager\.clearSession\(\)[\s\S]*unregisterTokenForProfileAfterLogout\(profileId\)/);
  assert.match(androidPushTokenManager, /fun unregisterTokenForProfileAfterLogout\(profileId: String\?\)/);
  assert.match(iosAuthRepository, /val bearerToken = session\.currentSession\(\)\?\.bearerToken[\s\S]*session\.clear\(\)[\s\S]*post\(/);
