...
test("profile role mutation failure and retry stay focal and cross-platform", () => {
  ...
  assert.match(iosTest, /QUATA_IOS_PROFILE_ROLES_FORCE_FAILURE[\s\S]*let forcedError = profileElement\("public-profile\.error\.[\s\S]*official\.isEnabled[\s\S]*forcedError\.waitForNonExistence/);
