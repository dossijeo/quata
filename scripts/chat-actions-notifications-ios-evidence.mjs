...
try {
  ...
  } else if (!translationOnly && !profileEvidenceOnly && !conversationCreateOnly && !messagesLifecycleOnly && !communityChatOnly && !menuSurfaceOnly && !muteNegativeOnly && !keyboardMenuOnly && !attachmentsAudioOnly && !documentActionsOnly && !composerEmojiOnly && !groupSosOnly && !attachmentPickerOnly) {
  ...
  state.editableMarker = translationOnly || profileEvidenceOnly || conversationCreateOnly || messagesLifecycleOnly || communityChatOnly || menuSurfaceOnly || muteNegativeOnly || keyboardMenuOnly || attachmentsAudioOnly || documentActionsOnly || composerEmojiOnly || groupSosOnly || attachmentPickerOnly || groupAdminOnly || groupModerationOnly ? null : `chat-actions-ios-editable-${randomUUID()}`;
  state.composerMarker = translationOnly || profileEvidenceOnly || communityChatOnly || menuSurfaceOnly || muteNegativeOnly || attachmentsAudioOnly || documentActionsOnly || groupSosOnly || attachmentPickerOnly || groupAdminOnly || groupModerationOnly ? null : `🚨 chat-actions-ios-send-${randomUUID()} www.quata.test/chat 📝`;
  state.replyMarker = translationOnly || profileEvidenceOnly || conversationCreateOnly || messagesLifecycleOnly || communityChatOnly || menuSurfaceOnly || muteNegativeOnly || keyboardMenuOnly || attachmentsAudioOnly || documentActionsOnly || composerEmojiOnly || groupSosOnly || attachmentPickerOnly || groupAdminOnly || groupModerationOnly ? null : `chat-actions-ios-reply-${randomUUID()}`;
  state.editMarker = translationOnly || profileEvidenceOnly || conversationCreateOnly || messagesLifecycleOnly || communityChatOnly || menuSurfaceOnly || muteNegativeOnly || keyboardMenuOnly || attachmentsAudioOnly || documentActionsOnly || composerEmojiOnly || groupSosOnly || attachmentPickerOnly || groupAdminOnly || groupModerationOnly ? null : `chat-actions-ios-edit-${randomUUID()}`;
  ...
  if (!translationOnly) {
    ...
    if (!profileEvidenceOnly && !conversationCreateOnly && !messagesLifecycleOnly && !communityChatOnly && !menuSurfaceOnly && !muteNegativeOnly && !keyboardMenuOnly && !attachmentsAudioOnly && !documentActionsOnly && !composerEmojiOnly && !groupSosOnly && !attachmentPickerOnly && !groupAdminOnly && !groupModerationOnly) {
...
`, 30 * 60 * 1000).catch(async (error) => {
    ...
    report.fixture = conversationCreateOnly
      ...
      : (profileEvidenceOnly || messagesLifecycleOnly || communityChatOnly || menuSurfaceOnly || muteNegativeOnly || keyboardMenuOnly || attachmentsAudioOnly || documentActionsOnly || composerEmojiOnly || groupSosOnly || attachmentPickerOnly || groupAdminOnly || groupModerationOnly)
      ? {
        threadId: state.thread,
        conversationId: `sb:${state.thread}`,
