...
const CHECK = "POST-IMAGE-EDITOR-ANDROID-REAL-001";
const DEFAULT_CREDENTIALS_FILE = "C:/Users/PC/QUATA_CHAT_GROUP_CREDENTIALS_FILE.txt";
const deviceCredentialsPath = "app-internal:post-image-editor-credentials.json";
const deviceTempCredentialsPath = "/data/local/tmp/post-image-editor-credentials.json";
...
try {
  ...
  localCredentials = join("build-reports", "android", `post-image-editor-credentials-${randomUUID()}.json`);
  ...
  const instrumentationOutput = await runCapture(adb, [
    ...
    "-e", "class", "com.quata.feature.postcomposer.presentation.PostPublishRealInstrumentedTest#authenticatedUserExercisesPostImageEditorFromCommonComposer",
    "-e", "quataPostPublishCredentialsFile", deviceCredentialsPath,
    "-e", "quataPostImageEditorEvidence", "1",
    "com.quata.test/androidx.test.runner.AndroidJUnitRunner",
  ]);
  const attempt = { source: "gallery-image", outcome: "success", instrumentationTail: redactedTail(instrumentationOutput) };
  ...
  console.log(`Post image editor Android evidence written: ${options.output}`);
}

if (report.status !== "passed") {
  console.error(`Post image editor Android evidence failed: ${report.error?.message ?? report.error ?? "unknown"}.`);
  process.exitCode = 1;
} else {
  console.log("Post image editor Android evidence passed.");
}

function parseArgs(args) {
  const parsed = {
    output: resolve(join("build-reports", "android", "post-image-editor-evidence.json")),
    evidenceDir: resolve(join("build-reports", "android", "post-image-editor-evidence")),
