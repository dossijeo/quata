...
$networkName = "quata-follows-net-$suffix"
$dbContainer = "quata-follows-db-$suffix"
$apiContainer = "quata-follows-api-$suffix"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "The isolated database contract failed before PostgREST."
    }
    ...
    docker run `
        ...
        --volume "${repoRoot}:/workspace:ro" `
        node:22-alpine `
        node /workspace/scripts/community-profile-follows-postgrest.test.mjs `
