#!/usr/bin/env bash
# Opt-in local gate: seed a real Keychain session, then execute Chat translation UI evidence.
set -euo pipefail
...
: "${QUATA_IOS_CHAT_TRANSLATION_LOG_DIR:=build/reports/ios/chat-translation}"
: "${QUATA_IOS_CHAT_TRANSLATION_RESULT_BUNDLE_DIR:=}"
...
mkdir -p "$QUATA_IOS_CHAT_TRANSLATION_LOG_DIR"
...
timeout_diagnostics() {
  local label="$1" diagnostics="$QUATA_IOS_CHAT_TRANSLATION_LOG_DIR/${label}-timeout-diagnostics.log"
  {
...
set +e
run_bounded bootstatus 120 "$QUATA_IOS_CHAT_TRANSLATION_LOG_DIR/bootstatus.log" \
  xcrun simctl bootstatus "$QUATA_IOS_SIMULATOR_UDID" -b
bootstatus_status=$?
set -e
if [[ "$bootstatus_status" -eq 124 ]]; then
  devices_json="$QUATA_IOS_CHAT_TRANSLATION_LOG_DIR/simulator-devices-after-bootstatus-timeout.json"
...
ui='QuataIosUITests/QuataIosAuthenticatedChatTranslationUITests/testRealChatMessageTranslatesAndReturnsToTheConversation'
ui_method='testRealChatMessageTranslatesAndReturnsToTheConversation'

run_and_require() {
  local selected="$1" method="$2" log="$3"
  local result_args=()
  if [[ -n "$QUATA_IOS_CHAT_TRANSLATION_RESULT_BUNDLE_DIR" ]]; then
    mkdir -p "$QUATA_IOS_CHAT_TRANSLATION_RESULT_BUNDLE_DIR"
    local result_bundle="$QUATA_IOS_CHAT_TRANSLATION_RESULT_BUNDLE_DIR/${method}.xcresult"
    rm -rf "$result_bundle"
    result_args=(-resultBundlePath "$result_bundle")
  fi
  run_bounded "$method" 600 "$log" \
...
run_and_require "$seed" testSeedAuthenticatedSessionForVisualGates "$QUATA_IOS_CHAT_TRANSLATION_LOG_DIR/seed.log"
run_and_require "$ui" "$ui_method" "$QUATA_IOS_CHAT_TRANSLATION_LOG_DIR/ui.log"
echo "CHAT_TRANSLATION_IOS_UI_GATE_PASSED" >&2
