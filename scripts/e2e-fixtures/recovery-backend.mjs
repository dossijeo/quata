...
import {recordRecoverySession,recordRecoveryNativeSession,createRecoveryWebActorVerifier} from "./recovery-session-receipt.mjs";
...
export function createRecoveryBackend({client,journal,record,backendUrl,publicKey,pageOperationsSettled,
  confirmPreviousRunSettled,fetchImpl=fetch,sessionKind="web"}) {
  const root=new URL(backendUrl);
  if(root.protocol!=="https:" || root.username || root.password || root.pathname!=="/" || root.search || root.hash ||
      typeof pageOperationsSettled!=="function" || !["web","native"].includes(sessionKind))throw Error("recovery_backend_configuration_invalid");
  ...
  const durableTicket=async ticket=>{
    ...
    const native=ticket.kind==="native";
    if((ticket.kind!==undefined && ticket.kind!=="native") || (native ? "native" : "web")!==sessionKind || (native &&
        (typeof ticket.ticketId!=="string" || ticket.clientInstanceId!==undefined || ticket.webSessionId!==undefined)))throw Error("recovery_ticket_kind_mismatch");
    const durable=await journal.read();
    if(!sameActor(durable))throw Error("recovery_journal_actor_mismatch");
    const matches=durable.state.sessions.filter(item=>native ? item.kind==="native" && item.ticketId===ticket.ticketId :
      item.kind!=="native" && item.clientInstanceId===ticket.clientInstanceId);
    if(matches.length!==1 || !sameActor(matches[0]) || matches[0].purpose!==ticket.purpose)throw Error("recovery_ticket_missing");
    if(native && (matches[0].clientInstanceId!==undefined || matches[0].webSessionId!==undefined))throw Error("recovery_ticket_kind_mismatch");
  ...
  const receipt=async ticket=>{
    ...
    if(!entry.authSessionId || (entry.kind!=="native" && !entry.webSessionId))throw Error("recovery_ticket_receipt_missing");
    return entry;
  };
  ...
  return Object.freeze({
    verifyActor:sessionKind==="web" ? createRecoveryWebActorVerifier({client,journal,backendUrl,publicKey,fetchImpl}) : async(value,ticket,credentials)=>{
      if(!sameActor(value) || credentials?.profileId!==record.profileId)throw Error("recovery_session_actor_mismatch");
      await durableTicket(ticket);
      return recordRecoveryNativeSession({client,journal,ticket,backendUrl,publicKey,fetchImpl,accessToken:credentials.accessToken});
    },
    ...
    async planSession(value){
      ...
      return {runId:record.runId,profileId:record.profileId,authUserId:record.authUserId,purpose:value.purpose,
        ...(sessionKind==="native" ? {kind:"native",ticketId:randomUUID()} : {clientInstanceId:randomUUID()})};
    },
    ...
    async verifyLogin(password,ticket){
      ...
      const result=await post({action:sessionKind==="native" ? "login" : "web_login",...identity,password,
        ...(sessionKind==="web" ? {client_instance_id:ticket.clientInstanceId} : {})});
      ...
      if(sessionKind==="native")return recordRecoveryNativeSession({client,journal,ticket,backendUrl,publicKey,fetchImpl,
    ...
    async revokeSessions(tickets){
      ...
      for(const ticket of tickets){
        ...
        try {
          if(entry.kind!=="native")await client.query(`update public.web_client_sessions set revoked_at=coalesce(revoked_at,now())
    ...
    async sessionsClean(tickets){
      for(const ticket of tickets){
        ...
        if(verified.kind==="native"){
          const result=await client.query(`select count(*)::int as auth_count from auth.sessions where id=$1::uuid and user_id=$2::uuid`,
            [verified.authSessionId,record.authUserId]);
          if(result.rows[0]?.auth_count!==0)return false;
          continue;
        }
