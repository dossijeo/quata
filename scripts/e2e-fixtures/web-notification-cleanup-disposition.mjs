...
async function audit(client,record,proof,replyId=null) {
  ...
  if(participants.rowCount!==2||participants.rows.some(row=>row.left_at!==null)||
  ...
  const result=await client.query(`select
    ...
    not exists(select 1 from public.push_delivery_log where message_id in
      (select id from public.chat_messages where thread_id=$3::bigint)) as native_logs,
    not exists(select 1 from public.web_push_delivery_log where message_id=$4::bigint) as reply_web_logs`,
    [proof.peerAuthId,proof.peerId,proof.threadId,replyId]);
...
export async function assertWebReplyCleanupDisposition({client,journal,peerJournal,dispatcherFingerprint,disposition}) {
  try {
    ...
    await audit(client,record,expected,receipt.messageId);
