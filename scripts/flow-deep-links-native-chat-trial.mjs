...
export async function runNativeDeepLinkChatTrial({client,privateDirectory,backendUrl,publicKey,
  ...
  if(!['cold','warm'].includes(mode)||!path.isAbsolute(privateDirectory)||
  ...
  try {
    ...
    const ticket={runId,profileId:actor.record.profileId,authUserId:actor.record.authUserId,kind:'native',ticketId:randomUUID(),
      purpose:'deep_link',requestStarted:true,androidNativeLogin:{observationVerified:false}};
    ...
    report.observation=await ui.login({runId,stepId:randomUUID(),countryCode:actor.record.countryCode,
      phone:actor.record.phone,password:actor.record.password,messageId:String(target.messageId)});
    if(report.observation?.passed!==true)throw Error('deep_link_native_observation_failed');
    await ui.close();
    await checkpoint(actor,state=>{state.sessions[0].androidNativeLogin.observationVerified=true;});
    ...
    await checkpoint(actor,state=>{state.sessions[0].androidNativeLogin.read={input,started:true,verified:false};});
    ...
    await checkpoint(actor,state=>{state.sessions[0].androidNativeLogin.read.privateSession=session;});
    ...
    await checkpoint(actor,state=>{state.sessions[0].androidNativeLogin.read.verified=true;});
    const clearInput={...session,runId,stepId:randomUUID(),stage:'clear'};
    report.phase='native_session_clear';
    await checkpoint(actor,state=>{state.sessions[0].androidNativeLogin.clear={input:clearInput,started:true,verified:false};});
    ...
    await checkpoint(actor,state=>{state.sessions[0].androidNativeLogin.clear.verified=true;});
    ...
    if(await settled().catch(()=>false))try {
      ...
      for(const actor of [...actors].reverse()) {
        ...
        if(saved.state.sessions.some(s=>s.androidNativeLogin))await retireAndroidDeepLinkResidue({client,journal:actor.journal,record:actor.record,operationsSettled:settled});
