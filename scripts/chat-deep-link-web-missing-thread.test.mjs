...
for(const kind of ["expected-rejection","other-thread-success","other-thread-error","network-failure","private-content","technical-error"])test(`Chrome missing thread ${kind}`,{skip:!modulePath||!chrome,timeout:45000},async()=>{
  await mkdir(root,{recursive:true});const directory=await mkdtemp(path.join(root,"test-"));
  const backend=createServer((req,res)=>{
    if(req.url.includes('other=1')){
      res.writeHead(kind==='other-thread-error'?503:200,{'Access-Control-Allow-Origin':'*','Access-Control-Allow-Headers':'content-type','Cross-Origin-Resource-Policy':'cross-origin','Content-Type':'application/json'}).end(JSON.stringify({thread:{id:123},messages:[]}));return;
    }
  ...
  try {
    await writeFile(path.join(directory,"index.html"),`<!doctype html><html><body><main></main><script>
      ...
      async function render(){const detail=location.hash.includes('chat-sb%3A999');document.documentElement.setAttribute('data-quata-shell-route',detail?'chat/sb:999':'chat');
        ...
        if(detail){document.getElementById('chat.back').onclick=()=>{rpc('quata_chat_cleanup_empty_private_thread');location.hash='chat';};
          ${kind.startsWith('other-thread')?`await fetch('${backendUrl}/rest/v1/rpc/quata_chat_get_thread?other=1',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({p_actor_profile_id:'owned',p_thread_id:123})});`:''}
          await rpc('quata_chat_get_thread');await rpc('quata_chat_mark_thread_read');}}
    ...
    if(kind==="expected-rejection"||kind.startsWith('other-thread')) {const result=await operation;assert.equal(result.passed,true);assert.equal(result.observations.length,2);assert.equal(adapter.operationsSettled(),kind!=='other-thread-error');
