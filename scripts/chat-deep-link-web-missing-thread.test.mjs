...
for(const kind of ["expected-rejection","network-failure","private-content"])test(`Chrome missing thread ${kind}`,{skip:!modulePath||!chrome,timeout:45000},async()=>{
  ...
  try {
    await writeFile(path.join(directory,"index.html"),`<!doctype html><html><body><main></main><script>
      ...
      async function render(){const detail=location.hash.includes('chat-sb%3A999');document.documentElement.setAttribute('data-quata-shell-route',detail?'chat/sb:999':'chat');
        document.querySelector('main').innerHTML=detail?'<button>Reintentar mensajes</button><button id="chat.back">Back</button>${kind==="private-content"?'<button id="chat.message.456">Private</button>':''}':'Conversation list';
    ...
    else {await assert.rejects(operation,/deep_link_web_missing_thread_read_failed/);if(kind==="network-failure")assert.equal(adapter.operationsSettled(),false);}
