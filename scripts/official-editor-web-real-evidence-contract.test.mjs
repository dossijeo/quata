...
test("Official editor Web real evidence is opt-in, redacted, and reversible", () => {
  ...
  assert.match(runner, /officialEditorSemanticClick\(page, "official-editor-mode-switch"\)/);
  assert.match(runner, /officialEditorSemanticInput\(page, "official-editor-advanced-title", titleText\)/);
  assert.match(runner, /officialEditorSemanticInput\(page, "official-editor-advanced-summary", summaryText\)/);
  assert.match(runner, /async function officialEditorSemanticClick\(page, target\)/);
  assert.match(runner, /bridge\.semanticClick\(String\(target \?\? ""\)\) === true/);
  assert.match(runner, /async function officialEditorSemanticInput\(page, target, value\)/);
  ...
  assert.match(runner, /quata-portable-rich-text-toolbar-heading/);
  assert.match(runner, /quata-portable-rich-text-heading-\$\{level\}/);
  ...
  assert.match(runner, /\(!box \|\| box\.width <= 0 \|\| box\.height <= 0\) && id === "official-editor-publish"/);
  assert.match(runner, /await clickWebWasmVisualPublishFallback\(page\)/);
  assert.match(runner, /page\.mouse\.click\(Math\.round\(box\.x \+ box\.width \/ 2\), Math\.round\(box\.y \+ box\.height \/ 2\)\)/);
  ...
  assert.doesNotMatch(runner, /officialEditorAction\(page, "setAdvancedMode"/);
  ...
  assert.match(webBridge, /semanticClick: \(target\) => semanticClick\(String\(target \?\? ''\)\) === true/);
  assert.match(webBridge, /semanticInput: \(target, value\) => semanticInput\(String\(target \?\? ''\), String\(value \?\? ''\)\) === true/);
  assert.match(webBridge, /__quataOfficialRichTextEditorE2eProduct/);
  assert.match(webBridge, /data-quata-official-rich-text-editor-e2e/);
