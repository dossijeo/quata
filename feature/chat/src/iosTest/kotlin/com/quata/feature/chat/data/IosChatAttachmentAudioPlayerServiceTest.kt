...
import com.quata.feature.chat.presentation.chat.ChatAudioPlaybackController
...
class IosChatAttachmentAudioPlayerServiceTest {
    ...
    fun dispose_cancelsInFlightLoadStopsPlayerAndLeavesNoTemporaryFile() = runBlocking {
        ...
        val controller = ChatAudioPlaybackController(
            audioPlayer = service,
            messages = { listOf(audioMessage("pending")) },
            dispatcher = Dispatchers.Unconfined,
        )

        controller.toggle(audioMessage("pending"), remoteFile("pending"))
    ...
    private fun audioMessage(id: String) = Message(
        id = id,
        ...
        sentAt = "",
        sentAtMillis = 1_000,
        attachmentUri = "https://quata.test/$id",
        attachmentName = id,
    ...
    private class FakePlayer(private val calls: MutableList<String>) : AudioPlayerService {
        ...
        override suspend fun stop(): PlatformResult<Unit> {
            calls += "stop"
            if (suspendLoad && calls.count { it == "stop" } > 1) terminalStop.complete(Unit)
            return stopResult
        }
