...
internal enum class RichTextBlockType {
    Paragraph,
    Heading1,
    Heading2,
    Heading3,
...
internal enum class InlineFormat {
    Bold,
    Italic,
    Underline,
    Strike,
...
internal class QuataRichTextBlock(
...
internal class QuataRichTextEditorState(initialHtml: String = "") {
    ...
    internal fun isBlockSelected(blockId: String): Boolean = selectedBlockIds.contains(blockId)

    internal fun captureDragSnapshot(): QuataRichTextDragSnapshot {
    ...
    internal fun restoreFromDragSnapshot(snapshot: QuataRichTextDragSnapshot, pushHistory: Boolean = false) {
    ...
    internal fun getDragPayloadSnapshot(anchorBlockId: String? = null): QuataRichTextDragPayload? {
    ...
    internal fun startDragSession(anchorBlockId: String): Boolean {
    ...
    internal fun updateDragSession(
        targetIndex: Int?,
        futureRootIndent: Int?,
    ...
    internal fun completeDragSession(): Boolean {
        val session = dragMachine ?: return false
    ...
    internal fun cancelDragSession() {
        val session = dragMachine ?: return
        restoreFromDragSnapshot(session.dragSnapshot, pushHistory = false)
        dragMachine = null
    }

    internal fun isDragSessionActive(): Boolean = dragMachine != null

    internal fun currentDragMachineTargetIndex(): Int? = dragMachine?.targetIndex

    internal fun currentDragMachineFutureRootIndent(): Int? = dragMachine?.futureRootIndent

    internal fun isBlockInDragPayload(blockId: String): Boolean {
...
internal data class QuataRichTextDragSnapshot(
...
internal data class QuataTextDragMachine(
...
internal data class QuataRichTextBlockSnapshot(
    val id: String,
...
internal data class QuataRichTextDragPayload(
    val primaryRootId: String,
...
internal data class QuataTextSlashSession(
    val blockId: String,
    val slashStart: Int,
