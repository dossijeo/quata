...
internal fun convertBlocksToHtml(blocks: List<QuataRichTextBlock>): String {
...
internal fun parseHtmlToRichTextBlocks(rawHtml: String): List<QuataRichTextBlock> {
...
internal fun sanitizeGeneratedHtml(raw: String): String {
    ...
    return tagPattern.replace(sanitized) { match ->
        val name = match.groupValues[1].lowercase(Locale.getDefault())
        if (AllowedHtmlTags.contains(name)) match.value else ""
    }
}

internal fun sanitizeHref(rawHref: String?): String? {
...
private fun parseInlineHtml(raw: String): QuataInlineParseResult {
    ...
    for (match in tokenPattern.findAll(raw)) {
        ...
        val tag = match.groupValues.getOrNull(1).orEmpty().lowercase(Locale.ROOT)
